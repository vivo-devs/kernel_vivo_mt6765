...
static void send_key_event(u32 keycode, u32 flag)
{
	...
	case DW_KEY:
		...
		snd_soc_jack_report(&accdet->jack, report,
				SND_JACK_BTN_1);
		pr_info("%s : BSP.Audio send_key_event SND_JACK_BTN_1\n", __func__);
		break;
	case UP_KEY:
		if (flag != 0)
		...
		snd_soc_jack_report(&accdet->jack, report,
				SND_JACK_BTN_2);
		pr_info("%s : BSP.Audio send_key_event SND_JACK_BTN_2\n", __func__);
		break;
	case MD_KEY:
		if (flag != 0)
		...
		snd_soc_jack_report(&accdet->jack, report,
				SND_JACK_BTN_0);
		pr_info("%s : BSP.Audio send_key_event SND_JACK_BTN_0\n", __func__);
		break;
	case AS_KEY:
		if (flag != 0)
		...
		snd_soc_jack_report(&accdet->jack, report,
				SND_JACK_BTN_3);
		pr_info("%s : BSP.Audio send_key_event SND_JACK_BTN_3\n", __func__);
		break;
	}
}
...
static void multi_key_detection(u32 cur_AB)
{
	...
	pr_info("%s : BSP.Audio multi_key_detection cur_AB = %d ,accdet->cur_key = %d\n", __func__, cur_AB, accdet->cur_key);
...
static void eint_work_callback(struct work_struct *work)
{
	...
	if (!gpio_is_valid(accdet->r_gpiopin)) {
		if (accdet->cur_eint_state == EINT_PLUG_IN) {
			...
			accdet_update_bits(ACCDET_CMP_PWM_EN_ADDR,
					ACCDET_CMP_PWM_EN_SFT, 0x7, 0x7);
			pr_info("%s : BSP.Audio eint_work_callback EINT_PLUG_IN enable_accdet\n", __func__);
			enable_accdet(0);
		} else {
			mutex_lock(&accdet->res_lock);
			accdet->eint_sync_flag = false;
			...
			accdet_clear_bits(ACCDET_CMP_PWM_IDLE_ADDR,
				ACCDET_CMP_PWM_IDLE_SFT, 0x7, 0x7);
			pr_info("%s : BSP.Audio eint_work_callback EINT_PLUG_OUT disable_accdet\n", __func__);
			disable_accdet();
			headset_plug_out();
		}
	} else {
...
static void accdet_work_callback(struct work_struct *work)
{
	...
	check_cable_type();
	pr_info("%s : BSP.Audio accdet_work_callback check_cable_type\n", __func__);
	mutex_lock(&accdet->res_lock);
	if (accdet->eint_sync_flag) {
...
static void accdet_queue_work(void)
{
	...
	if (accdet->accdet_status == MIC_BIAS) {
		...
		pr_info("%s : BSP.Audio accdet_queue_work accdet_get_auxadc\n", __func__);
	}
...
static int pmic_eint_queue_work(int eintID)
{
	...
	if (HAS_CAP(accdet->data->caps, ACCDET_PMIC_EINT0)) {
		if (eintID == PMIC_EINT0) {
			if (accdet->cur_eint_state == EINT_PLUG_IN) {
				pr_info("%s : BSP.Audio pmic_eint_queue_work accdet->cur_eint_state == EINT_PLUG_IN\n", __func__);
...
void accdet_irq_handle(void)
{
	...
	if ((irq_status & ACCDET_IRQ_MASK_SFT) && (eintID == 0)) {
		pr_info("%s : BSP.Audio accdet_irq_handle eintID == 0\n", __func__);
		clear_accdet_int();
		accdet_queue_work();
		clear_accdet_int_check();
	} else if (eintID != NO_PMIC_EINT) {
		pr_info("%s : BSP.Audio IRQ:0x%x, eint-%s trig. cur_eint_state:%d\n",
		__func__, irq_status,
		...
		pr_info("%s : BSP.Audio accdet_irq_handle eint_polarity_reverse,eintID = %d\n", __func__, eintID);
		eint_polarity_reverse(eintID);
		clear_accdet_eint(eintID);
		clear_accdet_eint_check(eintID);
...
static int accdet_get_dts_data(void)
{
	...
	if (HAS_CAP(accdet->data->caps, ACCDET_FOUR_KEY)) {
		...
		pr_info("%s: BSP.Audio headset-four-key-threshold: %d %d %d %d %d\n",
					 __func__, four_key[0], four_key[1], four_key[2], four_key[3], four_key[4]);
		if (!ret)
		...
		if (HAS_CAP(accdet->data->caps, ACCDET_THREE_KEY)) {
			...
			pr_info("%s: BSP.Audio headset-three-key-threshold: %d %d %d %d\n",
					 __func__, three_key[0], three_key[1], three_key[2], three_key[3]);
		} else {
			ret = of_property_read_u32_array(node,
				"headset-three-key-threshold-CDD", three_key,
				ARRAY_SIZE(three_key));
			pr_info("%s: BSP.Audio headset-three-key-threshold-CDD: %d %d %d %d\n",
					 __func__, three_key[0], three_key[1], three_key[2], three_key[3]);
		}
		if (!ret)
