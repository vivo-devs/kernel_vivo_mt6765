			...
			if (!strncmp(project_name, "PD2140F_EX", 10)) {
				...
				if (board_version) {
					if (*(board_version + 5) == '0') {
						fpsensor_debug(ERR_LOG, "%s(), not support fingerprint,do not load icnf driver", __func__);
						return -1;
					}
					fpsensor_debug(ERR_LOG, "%s(), support fingerprint, continue following process.", __func__);
				} else {
					fpsensor_debug(ERR_LOG, "%s(), board_version is NULL.", __func__);
					return -1;
				}
			}
		} else {
			fpsensor_debug(ERR_LOG, "%s(), can not find project-name", __func__);
		}
