...
extern int incell_tp_reset_output_paneloff(int enable);
...
static int focaltech_mode_change(int which)
{
	...
	if (which == TOUCHSCREEN_SLEEP) {
		...
		if (last_ts_state == TOUCHSCREEN_NORMAL) {
			...
			incell_tp_reset_output_paneloff(0);
			mdelay(5);
			incell_tp_reset_output_paneloff(1);
...
extern int fts_imei_read_8006p_noflash(u8 *imei);
/*
...
static int fts_ts_probe(struct spi_device *spi)
{
	...
	vts_data->readImei= fts_imei_read_8006p_noflash;
	...
	vivoTsFwAdd(VTS_FW_TYPE_FW, VIVO_CUST_FW_FT8006P_SPI_TRULY, sizeof(VIVO_CUST_FW_FT8006P_SPI_TRULY), "PD1901", VTS_FW_NOPIN_VALUE, VTS_MVC_TMA, 0x15);
	vivoTsFwAdd(VTS_FW_TYPE_FW, VIVO_CUST_FW_FT8006P_SPI_TRULY_BOE, sizeof(VIVO_CUST_FW_FT8006P_SPI_TRULY_BOE), "PD1901", VTS_FW_NOPIN_VALUE, VTS_MVC_TMA, 0x3C);
	vivoTsFwAdd(VTS_FW_TYPE_FW, VIVO_CUST_FW_FT8006P_SPI_TRULY_AUO, sizeof(VIVO_CUST_FW_FT8006P_SPI_TRULY_AUO), "PD1901", VTS_FW_NOPIN_VALUE, VTS_MVC_TMA, 0x40);	
	vivoTsFwAdd(VTS_FW_TYPE_FW, VIVO_CUST_FW_FT8006P_SPI_TRULY_LC, sizeof(VIVO_CUST_FW_FT8006P_SPI_TRULY_LC), "PD1901", VTS_FW_NOPIN_VALUE, VTS_MVC_TMA, 0x50);
...
static int __init fts_ts_init(void)
{
	...
	if (!strcmp(productName, "PD1901")) {
		if ((0x15 != get_lcm_id()) && (0x3C != get_lcm_id()) && (0x40 != get_lcm_id()) && (0x50 != get_lcm_id())) {
			VTI("not 8006P");
			return 0;
		}
	} else {
		VTI("PD1987F_EX, not 8006p");
		return 0;
	}
