...
int focaltech_input_method_ctl(int state) 
{
	int i = 0;
	u8 temp_data = 0;
	int ret = 0;
	if ((0x15 == report_lcm_id()) || (0x40 == report_lcm_id())) {
		for (i = 0; i < 10; i++) {
			ret = fts_write_reg_byte(0xCA, state);
			if (ret < 0)
				VTI("fail to wirte 0xCA register %d", state);

			mdelay(5);

			ret = fts_read_reg_byte(0xCA, &temp_data);
			if (ret < 0)
				VTI("fail to read 0xCA register %d", temp_data);

			if (temp_data != state) {
				VTI("Fail to write 0xCA.data = %d", state);
				msleep(10);
				continue;
			} else {
				VTI("FTS : Success to write 0x90b.data = %d", state);
				break;
			}
		}
	}
 ...
 int bbk_ft_process_by_package(unsigned char *package_name){
	...
	if (!strcmp(package_name, "INPUT_METHOD_ON")) {
		VTI("INPUT_METHOD_ON");
		focaltech_input_method_ctl(1);
	}
	if (!strcmp(package_name, "INPUT_METHOD_OFF")) {
		VTI("INPUT_METHOD_OFF");
		focaltech_input_method_ctl(0);
	}
