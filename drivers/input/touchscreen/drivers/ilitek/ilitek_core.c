...
int bbk_ili_get_rawordiff_data_V2(struct vts_device *vtsdev, enum vts_frame_type which, short *data, int size)
{
	...
	while ((count < frame_count) && (timeout > 0)) {
		ipio_info("count = %d, over = %d, frame = %d\n",
				count, count % 1024, idev_V2->debug_data_frame);

		if ((count % 1024) < idev_V2->debug_data_frame) {
			ilitek_plat_irq_disable_V2();
			mutex_lock(&idev_V2->debug_mutex);
			ptr = &idev_V2->debug_buf[count % 1024][35];
		...
		if (count % 1024 == 0 && idev_V2->debug_data_frame == 1024)
			idev_V2->debug_data_frame = 0;
