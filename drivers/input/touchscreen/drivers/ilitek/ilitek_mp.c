...
s32 *frame_buf_V2;
s32 *key_buf_V2;
s32 *frame1_cbk700_V2, *frame1_cbk250_V2, *frame1_cbk200_V2;
s32 *cap_dac_V2, *cap_raw_V2;
int g_ini_items_V2;
...
static void dump_benchmark_data(s32 *max_ptr, s32 *min_ptr)
{
	...
	if (ipio_debug_level_V2) {
		...
		for (i = 0; i < core_mp_V2.frame_len; i++) {
			pr_cont("%d, ", max_ptr[i]);
			if (i % core_mp_V2.xch_len == core_mp_V2.xch_len - 1)
				pr_cont("\n");
		}
...
void dump_node_type_buffer_V2(s32 *node_ptr, u8 *name)
{
	int i;
...
static int parser_get_ini_key_value(char *section, char *key, char *value)
{
	...
	for (i = 0; i < g_ini_items_V2; i++) {
		if (strcmp(section, ilitek_ini_file_data_V2[i].pSectionName) != 0)
			continue;

		if (strcmp(key, ilitek_ini_file_data_V2[i].pKeyName) == 0) {
			ipio_memcpy(value, ilitek_ini_file_data_V2[i].pKeyValue, ilitek_ini_file_data_V2[i].iKeyValueLen, PARSER_MAX_KEY_VALUE_LEN);
			ipio_debug(" value:%s , pKeyValue: %s\n", value, ilitek_ini_file_data_V2[i].pKeyValue);
			ret = 0;
			break;
		}
	}
...
void parser_ini_nodetype_V2(s32 *type_ptr, char *desp, int frame_len)
{
	int i = 0, j = 0, index1 = 0, temp, count = 0;
	char str[512] = {0}, record = ',';

	for (i = 0; i < g_ini_items_V2; i++) {
		if ((strstr(ilitek_ini_file_data_V2[i].pSectionName, desp) <= 0) ||
			strcmp(ilitek_ini_file_data_V2[i].pKeyName, NODE_TYPE_KEY_NAME) != 0) {
			continue;
		}

		record = ',';
		for (j = 0, index1 = 0; j <= ilitek_ini_file_data_V2[i].iKeyValueLen; j++) {
			if (ilitek_ini_file_data_V2[i].pKeyValue[j] == ';' || j == ilitek_ini_file_data_V2[i].iKeyValueLen) {
				if (record != '.') {
					memset(str, 0, sizeof(str));
					ipio_memcpy(str, &ilitek_ini_file_data_V2[i].pKeyValue[index1], (j - index1), sizeof(str));
					temp = katoi_V2(str);
				...
				record = ilitek_ini_file_data_V2[i].pKeyValue[j];
				index1 = j + 1;
			}
		}
	}
...
void parser_ini_benchmark_V2(s32 *max_ptr, s32 *min_ptr, int8_t type, char *desp, int frame_len)
{
	int i = 0, j = 0, index1 = 0, temp, count = 0;
	char str[512] = {0}, record = ',';
	s32 data[4];
	...
	for (i = 0; i < g_ini_items_V2; i++) {
		if ((strcmp(ilitek_ini_file_data_V2[i].pSectionName, benchmark_str) != 0) ||
			strcmp(ilitek_ini_file_data_V2[i].pKeyName, BENCHMARK_KEY_NAME) != 0)
			continue;

		record = ',';
		for (j = 0, index1 = 0; j <= ilitek_ini_file_data_V2[i].iKeyValueLen; j++) {
			if (ilitek_ini_file_data_V2[i].pKeyValue[j] == ',' || ilitek_ini_file_data_V2[i].pKeyValue[j] == ';' ||
				ilitek_ini_file_data_V2[i].pKeyValue[j] == '.' || j == ilitek_ini_file_data_V2[i].iKeyValueLen) {
...
/* char M_CFG_NIS = ':'; */
	...
	while (true) {
		...
		empty_section = 0;
		nodetype_flag = 0;
		if (g_ini_items_V2 > PARSER_MAX_KEY_NUM) {
		...
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL && ini_buf[n - 1] != M_CFG_SSR))) {
			...
			if (ini_buf[0] == M_CFG_SSL) {
				ilitek_ini_file_data_V2[g_ini_items_V2].iSectionNameLen = n - 2;
				if (ilitek_ini_file_data_V2[g_ini_items_V2].iSectionNameLen > PARSER_MAX_KEY_NAME_LEN) {
		...
		strcpy(ilitek_ini_file_data_V2[g_ini_items_V2].pSectionName, tmpSectionName);
		ilitek_ini_file_data_V2[g_ini_items_V2].iSectionNameLen = strlen(tmpSectionName);
		...
		if (isEqualSign == 0) {
			...
			if (strstr(ilitek_ini_file_data_V2[g_ini_items_V2].pSectionName, BENCHMARK_KEY_NAME) > 0) {
				banchmark_flag = 1;
				isEqualSign = -1;
			} else if (strstr(ilitek_ini_file_data_V2[g_ini_items_V2].pSectionName, NODE_TYPE_KEY_NAME) > 0) {
				nodetype_flag = 1;
				isEqualSign = -1;
			} else {
				continue;
		...
		if (banchmark_flag) {
			ilitek_ini_file_data_V2[g_ini_items_V2].iKeyNameLen = strlen(BENCHMARK_KEY_NAME);
			strcpy(ilitek_ini_file_data_V2[g_ini_items_V2].pKeyName, BENCHMARK_KEY_NAME);
			ilitek_ini_file_data_V2[g_ini_items_V2].iKeyValueLen = n;
		} else if (nodetype_flag) {
			ilitek_ini_file_data_V2[g_ini_items_V2].iKeyNameLen = strlen(NODE_TYPE_KEY_NAME);
			strcpy(ilitek_ini_file_data_V2[g_ini_items_V2].pKeyName, NODE_TYPE_KEY_NAME);
			ilitek_ini_file_data_V2[g_ini_items_V2].iKeyValueLen = n;
		} else{
			ilitek_ini_file_data_V2[g_ini_items_V2].iKeyNameLen = isEqualSign;
			if (ilitek_ini_file_data_V2[g_ini_items_V2].iKeyNameLen > PARSER_MAX_KEY_NAME_LEN) {
			...
			ipio_memcpy(ilitek_ini_file_data_V2[g_ini_items_V2].pKeyName, ini_buf,
						ilitek_ini_file_data_V2[g_ini_items_V2].iKeyNameLen, PARSER_MAX_KEY_NAME_LEN);
			ilitek_ini_file_data_V2[g_ini_items_V2].iKeyValueLen = n - isEqualSign - 1;
		}

		if (ilitek_ini_file_data_V2[g_ini_items_V2].iKeyValueLen > PARSER_MAX_KEY_VALUE_LEN) {
			ipio_err("MAX_KEY_VALUE_LEN: Out Of Length\n");
			ret = INI_ERR_OUT_OF_LINE;
			goto out;
		}

		ipio_memcpy(ilitek_ini_file_data_V2[g_ini_items_V2].pKeyValue,
			   ini_buf + isEqualSign + 1, ilitek_ini_file_data_V2[g_ini_items_V2].iKeyValueLen, PARSER_MAX_KEY_VALUE_LEN);

		ipio_debug("%s = %s\n", ilitek_ini_file_data_V2[g_ini_items_V2].pKeyName,
			ilitek_ini_file_data_V2[g_ini_items_V2].pKeyValue);
...
//	set_fs(get_ds());
	...
	for (i = 0; i < PARSER_MAX_KEY_NUM; i++) {
		memset(ilitek_ini_file_data_V2[i].pSectionName, 0, PARSER_MAX_KEY_NAME_LEN);
		memset(ilitek_ini_file_data_V2[i].pKeyName, 0, PARSER_MAX_KEY_NAME_LEN);
		memset(ilitek_ini_file_data_V2[i].pKeyValue, 0, PARSER_MAX_KEY_VALUE_LEN);
		ilitek_ini_file_data_V2[i].iSectionNameLen = 0;
		ilitek_ini_file_data_V2[i].iKeyNameLen = 0;
		ilitek_ini_file_data_V2[i].iKeyValueLen = 0;
	}
...
{
	int i, x, y;
	s32 *p_comb = frame_buf_V2;

	for (y = 0; y < core_mp_V2.ych_len; y++) {
		for (x = 0; x < core_mp_V2.xch_len; x++) {
			int tmp[4] = { 0 }, max = 0;
			int shift = y * core_mp_V2.xch_len;
			...
			} else if (y == (core_mp_V2.ych_len - 1) && x == 0) {
				tmp[0] = Mathabs(centre - p_comb[(shift - 1) + x]);	/* up */
				tmp[1] = Mathabs(centre - p_comb[shift + (x + 1)]);	/* right */
			} else if (y == 0 && x == (core_mp_V2.xch_len - 1)) {
				tmp[0] = Mathabs(centre - p_comb[(shift + 1) + x]);	/* down */
				tmp[1] = Mathabs(centre - p_comb[shift + (x - 1)]);	/* left */
			} else if (y == (core_mp_V2.ych_len - 1) && x == (core_mp_V2.xch_len - 1)) {
				tmp[0] = Mathabs(centre - p_comb[(shift - 1) + x]);	/* up */
				tmp[1] = Mathabs(centre - p_comb[shift + (x - 1)]);	/* left */
			} else if (y == 0 && x != 0) {
				tmp[0] = Mathabs(centre - p_comb[(shift + 1) + x]);	/* down */
	...
	if (strcmp(tItems[index].name, "open_integration") == 0) {
		...
		for (y = 0; y < core_mp_V2.ych_len; y++) {
			for (x = 0; x < core_mp_V2.xch_len; x++) {
				...
				for (k = 0; k < 8; k++) {
					if (((y + border_y[k] >= 0) && (y + border_y[k] < core_mp_V2.ych_len)) &&
								((x + border_x[k] >= 0) && (x + border_x[k] < core_mp_V2.xch_len))) {
						count++;
						sum += p_comb[(y + border_y[k]) * core_mp_V2.xch_len + (x + border_x[k])];
					}
				}
...
static void run_tx_rx_delta_test(int index)
{
	...
	for (y = 0; y < core_mp_V2.ych_len; y++) {
		for (x = 0; x < core_mp_V2.xch_len; x++) {
			...
			/* Tx Delta */
			if (y != (core_mp_V2.ych_len - 1))
				core_mp_V2.tx_delta_buf[shift + x] = Mathabs(p_comb[shift + x] - p_comb[(shift + 1) + x]);

			/* Rx Delta */
			if (x != (core_mp_V2.xch_len - 1))
				core_mp_V2.rx_delta_buf[shift + x] = Mathabs(p_comb[shift + x] - p_comb[shift + (x + 1)]);
		}
	}
}
				...
				s32 *min_ts, const char *desp)
{
	...
	for (y = 0; y < core_mp_V2.ych_len; y++) {
		...
		for (x = 0; x < core_mp_V2.xch_len; x++) {
			int shift = y * core_mp_V2.xch_len + x;
...
#define ADDR(x, y) ((y * core_mp_V2.xch_len) + (x))

int compare_charge_V2(s32 *charge_rate, int x, int y, s32 *inNodeType,
		int Charge_AA, int Charge_Border, int Charge_Notch)
{
	...
	for (k = 0; k < 8; k++) {
		...
		/*out of range */
		if ((tempX < 0) || (tempX >= core_mp_V2.xch_len) || (tempY < 0) || (tempY >= core_mp_V2.ych_len))
			continue;
...
int full_open_rate_compare_V2(s32 *full_open, s32 *cbk, int x, int y, s32 inNodeType, int full_open_rate)
{
	int ret = true;
...
void allnode_open_cdc_result_V2(int index, int *buf, int *dac, int *raw)
{
	int i;
	...
	if (strncmp(name, "open_integration_sp", strlen(name)) == 0) {
		for (i = 0; i < core_mp_V2.frame_len; i++)
			buf[i] = idev_V2->chip->open_sp_formula(dac[i], raw[i]);
	} else if (strncmp(name, "open test_c", strlen(name)) == 0) {
		for (i = 0; i < core_mp_V2.frame_len; i++)
			buf[i] = idev_V2->chip->open_c_formula(dac[i], raw[i], open_c_spec_V2.tvch - open_c_spec_V2.tvcl, open_c_spec_V2.gain);
	}
}
...
static int short_test(int index, int frame_index)
{
	...
	if (core_mp_V2.protocol_ver >= PROTOCOL_VER_540) {
		/* Calculate code to ohm and save to tItems[index].buf */
		for (j = 0; j < core_mp_V2.frame_len; j++)
			tItems[index].buf[frame_index * core_mp_V2.frame_len + j] = codeToOhm(frame_buf_V2[j], v_tdf, h_tdf);
	} else {
		for (j = 0; j < core_mp_V2.frame_len; j++)
			tItems[index].buf[frame_index * core_mp_V2.frame_len + j] = frame_buf_V2[j];
	}
...
static int allnode_key_cdc_data(int index)
{
	...
	len = core_mp_V2.key_len * 2;

	ipio_debug("Read key's length = %d\n", len);
	ipio_debug("core_mp_V2.key_len = %d\n", core_mp_V2.key_len);
	...
	*/
	/* Check busy */
	/*
	if (core_mp_V2.busy_cdc == POLL_CHECK)
		ret = ilitek_tddi_ic_check_busy_V2(50, 50);
	else
...
*/
	...
	//ret = idev_V2->read(ori, len);
	ret = ilitek_thp_send_cmd(cmd, core_mp_V2.cdc_len, ori, len);
	if (ret < 0) {
		ipio_err("Read cdc data error, len = %d\n", len);
		goto out;
	}

	ilitek_dump_data_V2(ori, 8, len, 0, "Key CDC original");

	if (key_buf_V2 == NULL) {
		key_buf_V2 = kcalloc(core_mp_V2.key_len, sizeof(s32), GFP_KERNEL);
		if (ERR_ALLOC_MEM(key_buf_V2)) {
			ipio_err("Failed to allocate FrameBuffer mem (%ld)\n", PTR_ERR(key_buf_V2));
			goto out;
		}
	} else {
		memset(key_buf_V2, 0x0, core_mp_V2.key_len);
	}
	...
	for (i = 0; i < core_mp_V2.frame_len; i++) {
		if (tItems[index].cmd == CMD_KEY_DAC) {
			...
			key_buf_V2[i] = (inDACp + inDACn) / 2;
		}
	}
	ilitek_dump_data_V2(key_buf_V2, 32, core_mp_V2.frame_len, core_mp_V2.xch_len, "Key CDC combined data");
...
static int allnode_open_cdc_data(int mode, int *buf)
{
	...
	len = (core_mp_V2.xch_len * core_mp_V2.ych_len * 2) + 2;
	...
	ilitek_dump_data_V2(cmd, 8, sizeof(cmd), 0, "Open SP command");
	...
	ilitek_dump_data_V2(ori, 8, len, 0, "Open SP CDC original");
	...
	ilitek_dump_data_V2(buf, 10, core_mp_V2.frame_len,  core_mp_V2.xch_len, "Open SP CDC combined");
out:
	atomic_set(&idev_V2->mp_int_check, DISABLE);
	ipio_kfree((void **)&ori);
	return ret;
...
static int allnode_mutual_cdc_data(int index)
{
	...
	ilitek_dump_data_V2(cmd, 8, core_mp_V2.cdc_len, 0, "Mutual CDC command");
	...
	ilitek_dump_data_V2(ori, 8, len, 0, "Mutual CDC original");

	if (frame_buf_V2 == NULL) {
		frame_buf_V2 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		if (ERR_ALLOC_MEM(frame_buf_V2)) {
			ipio_err("Failed to allocate FrameBuffer mem (%ld)\n", PTR_ERR(frame_buf_V2));
			goto out;
		}
	} else {
		memset(frame_buf_V2, 0x0, core_mp_V2.frame_len);
	}
	...
	for (i = 0; i < core_mp_V2.frame_len; i++) {
		if (tItems[index].cmd == CMD_MUTUAL_DAC) {
			...
			frame_buf_V2[i] = (inDACp + inDACn) / 2;
		} else {
			/* H byte + L byte */
			s32 tmp = (ori[(2 * i) + 1] << 8) + ori[(1 + (2 * i)) + 1];

			if ((tmp & 0x8000) == 0x8000)
				frame_buf_V2[i] = tmp - 65536;
			else
				frame_buf_V2[i] = tmp;
			...
			if (strncmp(tItems[index].name, "mutual_no_bk", strlen("mutual_no_bk")) == 0 ||
				strncmp(tItems[index].name, "mutual_no_bk_lcm_off", strlen("mutual_no_bk_lcm_off")) == 0) {
					frame_buf_V2[i] -= core_mp_V2.no_bk_shift;
			}
		}
	}

	ilitek_dump_data_V2(frame_buf_V2, 32, core_mp_V2.frame_len,	core_mp_V2.xch_len, "Mutual CDC combined");
	...
	for (y = 0; y < core_mp_V2.ych_len; y++) {
		for (x = 0; x < core_mp_V2.xch_len; x++) {
			...
			if (tItems[index].catalog == UNTOUCH_P2P)
				...
				if (core_mp_V2.tx_delta_buf[shift + x] < data[shift + x])
					core_mp_V2.tx_max_buf[shift + x] = data[shift + x];

				if (core_mp_V2.tx_delta_buf[shift + x] > data[shift + x])
					core_mp_V2.tx_min_buf[shift + x] = data[shift + x];

				/* Rx max/min comparison */
				if (core_mp_V2.rx_delta_buf[shift + x] < data[shift + x])
					core_mp_V2.rx_max_buf[shift + x] = data[shift + x];

				if (core_mp_V2.rx_delta_buf[shift + x] > data[shift + x])
					core_mp_V2.rx_min_buf[shift + x] = data[shift + x];
			} else {
...
static int create_mp_test_frame_buffer(int index, int frame_count)
{
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		if (core_mp_V2.tx_delta_buf == NULL) {
			core_mp_V2.tx_delta_buf = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
			if (ERR_ALLOC_MEM(core_mp_V2.tx_delta_buf)) {
				ipio_err("Failed to allocate tx_delta_buf mem\n");
				ipio_kfree((void **)&core_mp_V2.tx_delta_buf);
				return -ENOMEM;
			}
		}

		if (core_mp_V2.rx_delta_buf == NULL) {
			core_mp_V2.rx_delta_buf = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
			if (ERR_ALLOC_MEM(core_mp_V2.rx_delta_buf)) {
				ipio_err("Failed to allocate rx_delta_buf mem\n");
				ipio_kfree((void **)&core_mp_V2.rx_delta_buf);
				return -ENOMEM;
			}
		}

		if (core_mp_V2.tx_max_buf == NULL) {
			core_mp_V2.tx_max_buf = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
			if (ERR_ALLOC_MEM(core_mp_V2.tx_max_buf)) {
				ipio_err("Failed to allocate tx_max_buf mem\n");
				ipio_kfree((void **)&core_mp_V2.tx_max_buf);
				return -ENOMEM;
			}
		}

		if (core_mp_V2.tx_min_buf == NULL) {
			core_mp_V2.tx_min_buf = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
			if (ERR_ALLOC_MEM(core_mp_V2.tx_min_buf)) {
				ipio_err("Failed to allocate tx_min_buf mem\n");
				ipio_kfree((void **)&core_mp_V2.tx_min_buf);
				return -ENOMEM;
			}
		}

		if (core_mp_V2.rx_max_buf == NULL) {
			core_mp_V2.rx_max_buf = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
			if (ERR_ALLOC_MEM(core_mp_V2.rx_max_buf)) {
				ipio_err("Failed to allocate rx_max_buf mem\n");
				ipio_kfree((void **)&core_mp_V2.rx_max_buf);
				return -ENOMEM;
			}
		}

		if (core_mp_V2.rx_min_buf == NULL) {
			core_mp_V2.rx_min_buf = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
			if (ERR_ALLOC_MEM(core_mp_V2.rx_min_buf)) {
				ipio_err("Failed to allocate rx_min_buf mem\n");
				ipio_kfree((void **)&core_mp_V2.rx_min_buf);
				return -ENOMEM;
			}
		}
	} else {
		if (tItems[index].buf == NULL) {
			tItems[index].buf = vmalloc(frame_count * core_mp_V2.frame_len * sizeof(s32));
		...
		if (tItems[index].result_buf == NULL) {
			tItems[index].result_buf = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		...
		if (tItems[index].max_buf == NULL) {
			tItems[index].max_buf = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		...
		if (tItems[index].min_buf == NULL) {
			tItems[index].min_buf = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].bench_mark_max == NULL) {
				tItems[index].bench_mark_max = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
			...
			if (tItems[index].bench_mark_min == NULL) {
				tItems[index].bench_mark_min = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
...
static int mutual_test(int index)
{
	...
	for (y = 0; y < core_mp_V2.ych_len; y++) {
		for (x = 0; x < core_mp_V2.xch_len; x++) {
			if (tItems[i].catalog == TX_RX_DELTA) {
				core_mp_V2.tx_max_buf[y * core_mp_V2.xch_len + x] = INT_MIN;
				core_mp_V2.rx_max_buf[y * core_mp_V2.xch_len + x] = INT_MIN;
				core_mp_V2.tx_min_buf[y * core_mp_V2.xch_len + x] = INT_MAX;
				core_mp_V2.rx_min_buf[y * core_mp_V2.xch_len + x] = INT_MAX;
			} else {
				tItems[index].max_buf[y * core_mp_V2.xch_len + x] = INT_MIN;
				tItems[index].min_buf[y * core_mp_V2.xch_len + x] = INT_MAX;
			}
		}
	}
	...
	if (tItems[index].spec_option == BENCHMARK) {
		parser_ini_benchmark_V2(tItems[index].bench_mark_max, tItems[index].bench_mark_min,
								tItems[index].type_option, tItems[index].desp, core_mp_V2.frame_len);
	...
	for (i = 0; i < get_frame_cont; i++) {
		...
		default:
			for (j = 0; j < core_mp_V2.frame_len; j++)
				tItems[index].buf[i * core_mp_V2.frame_len + j] = frame_buf_V2[j];
			break;
		}
		compare_MaxMin_result(index, &tItems[index].buf[i * core_mp_V2.frame_len]);
	}
...
static int open_test_sp(int index)
{
	...
	if (frame1_cbk700_V2 == NULL) {
		frame1_cbk700_V2 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		if (ERR_ALLOC_MEM(frame1_cbk700_V2)) {
			ipio_err("Failed to allocate frame1_cbk700_V2 buffer\n");
			return -ENOMEM;
		}
	} else {
		memset(frame1_cbk700_V2, 0x0, core_mp_V2.frame_len);
	}

	if (frame1_cbk250_V2 == NULL) {
		frame1_cbk250_V2 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		if (ERR_ALLOC_MEM(frame1_cbk250_V2)) {
			ipio_err("Failed to allocate frame1_cbk250_V2 buffer\n");
			ipio_kfree((void **)&frame1_cbk700_V2);
			return -ENOMEM;
		}
	} else {
		memset(frame1_cbk250_V2, 0x0, core_mp_V2.frame_len);
	}

	if (frame1_cbk200_V2 == NULL) {
		frame1_cbk200_V2 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		if (ERR_ALLOC_MEM(frame1_cbk200_V2)) {
			ipio_err("Failed to allocate cbk buffer\n");
			ipio_kfree((void **)&frame1_cbk700_V2);
			ipio_kfree((void **)&frame1_cbk250_V2);
			return -ENOMEM;
		}
	} else {
		memset(frame1_cbk200_V2, 0x0, core_mp_V2.frame_len);
	}

	tItems[index].node_type = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
	...
	parser_ini_nodetype_V2(tItems[index].node_type, NODE_TYPE_KEY_NAME, core_mp_V2.frame_len);
	dump_node_type_buffer_V2(tItems[index].node_type, "node type");

	ret = parser_get_int_data(tItems[index].desp, "charge_aa", str);
	if (ret || ret == 0)
		Charge_AA = katoi_V2(str);

	ret = parser_get_int_data(tItems[index].desp, "charge_border", str);
	if (ret || ret == 0)
		Charge_Border = katoi_V2(str);

	ret = parser_get_int_data(tItems[index].desp, "charge_notch", str);
	if (ret || ret == 0)
		Charge_Notch = katoi_V2(str);

	ret = parser_get_int_data(tItems[index].desp, "full open", str);
	if (ret || ret == 0)
		full_open_rate = katoi_V2(str);
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		open[i].tdf_700 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].tdf_250 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].tdf_200 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].cbk_700 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].cbk_250 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].cbk_200 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].charg_rate = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].full_Open = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].dac = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
	}
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		allnode_open_cdc_result_V2(index, open[i].cbk_700, open[i].dac, open[i].tdf_700);
		allnode_open_cdc_result_V2(index, open[i].cbk_250, open[i].dac, open[i].tdf_250);
		allnode_open_cdc_result_V2(index, open[i].cbk_200, open[i].dac, open[i].tdf_200);

		addr = 0;

		/* record fist frame for debug_V2 */
		if (i == 0) {
			ipio_memcpy(frame1_cbk700_V2, open[i].cbk_700, core_mp_V2.frame_len * sizeof(s32), core_mp_V2.frame_len * sizeof(s32));
			ipio_memcpy(frame1_cbk250_V2, open[i].cbk_250, core_mp_V2.frame_len * sizeof(s32), core_mp_V2.frame_len * sizeof(s32));
			ipio_memcpy(frame1_cbk200_V2, open[i].cbk_200, core_mp_V2.frame_len * sizeof(s32), core_mp_V2.frame_len * sizeof(s32));
		}

		ilitek_dump_data_V2(open[i].cbk_700, 10, core_mp_V2.frame_len, core_mp_V2.xch_len, "cbk 700");
		ilitek_dump_data_V2(open[i].cbk_250, 10, core_mp_V2.frame_len, core_mp_V2.xch_len, "cbk 250");
		ilitek_dump_data_V2(open[i].cbk_200, 10, core_mp_V2.frame_len, core_mp_V2.xch_len, "cbk 200");
		...
		ilitek_dump_data_V2(open[i].charg_rate, 10, core_mp_V2.frame_len, core_mp_V2.xch_len, "origin charge rate");
		ilitek_dump_data_V2(open[i].full_Open, 10, core_mp_V2.frame_len, core_mp_V2.xch_len, "origin full open");

		addr = 0;
		for (y = 0; y < core_mp_V2.ych_len; y++) {
			for (x = 0; x < core_mp_V2.xch_len; x++) {
				if (full_open_rate_compare_V2(open[i].full_Open, open[i].cbk_700, x, y, tItems[index].node_type[addr], full_open_rate) == false) {
					tItems[index].buf[(i * core_mp_V2.frame_len) + addr] = 0;
		...
		ilitek_dump_data_V2(&tItems[index].buf[(i * core_mp_V2.frame_len)], 10, core_mp_V2.frame_len, core_mp_V2.xch_len, "after full_open_rate_compare_V2");

		addr = 0;
		for (y = 0; y < core_mp_V2.ych_len; y++) {
			for (x = 0; x < core_mp_V2.xch_len; x++) {
				tItems[index].buf[(i * core_mp_V2.frame_len) + addr] = compare_charge_V2(open[i].charg_rate, x, y, tItems[index].node_type, Charge_AA, Charge_Border, Charge_Notch);
				addr++;
			}
		}

		ilitek_dump_data_V2(&tItems[index].buf[(i * core_mp_V2.frame_len)], 10, core_mp_V2.frame_len, core_mp_V2.xch_len, "after compare charge rate");
...
static int open_test_cap(int index)
{
	...
	if (cap_dac_V2 == NULL) {
		cap_dac_V2 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		if (ERR_ALLOC_MEM(cap_dac_V2)) {
			ipio_err("Failed to allocate cap_dac_V2 buffer\n");
			return -ENOMEM;
		}
	} else {
		memset(cap_dac_V2, 0x0, core_mp_V2.frame_len);
	}

	if (cap_raw_V2 == NULL) {
		cap_raw_V2 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		if (ERR_ALLOC_MEM(cap_raw_V2)) {
			ipio_err("Failed to allocate cap_raw_V2 buffer\n");
			ipio_kfree((void **)&cap_dac_V2);
			return -ENOMEM;
		}
	} else {
		memset(cap_raw_V2, 0x0, core_mp_V2.frame_len);
	}
	...
	ipio_debug("open_test_c: frame_cont = %d, gain = %d, tvch = %d, tvcl = %d\n",
			tItems[index].frame_count, open_c_spec_V2.gain, open_c_spec_V2.tvch, open_c_spec_V2.tvcl);

	for (i = 0; i < tItems[index].frame_count; i++) {
		open[i].cap_dac_V2 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].cap_raw_V2 = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
		open[i].dcl_cap = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
	}

	for (i = 0; i < tItems[index].frame_count; i++) {
		ret = allnode_open_cdc_data(4, open[i].cap_dac_V2);
		if (ret < 0) {
			ipio_err("Failed to get Open CAP DAC data, %d\n", ret);
			goto out;
		}
		ret = allnode_open_cdc_data(5, open[i].cap_raw_V2);
		if (ret < 0) {
			ipio_err("Failed to get Open CAP RAW data, %d\n", ret);
			goto out;
		}

		allnode_open_cdc_result_V2(index, open[i].dcl_cap, open[i].cap_dac_V2, open[i].cap_raw_V2);

		/* record fist frame for debug_V2 */
		if (i == 0) {
			ipio_memcpy(cap_dac_V2, open[i].cap_dac_V2, core_mp_V2.frame_len * sizeof(s32), core_mp_V2.frame_len * sizeof(s32));
			ipio_memcpy(cap_raw_V2, open[i].cap_raw_V2, core_mp_V2.frame_len * sizeof(s32), core_mp_V2.frame_len * sizeof(s32));
		}

		ilitek_dump_data_V2(open[i].dcl_cap, 10, core_mp_V2.frame_len, core_mp_V2.xch_len, "DCL_Cap");

		addr = 0;
		for (y = 0; y < core_mp_V2.ych_len; y++) {
			for (x = 0; x < core_mp_V2.xch_len; x++) {
				tItems[index].buf[(i * core_mp_V2.frame_len) + addr] = open[i].dcl_cap[addr];
				addr++;
			}
		}
		compare_MaxMin_result(index, &tItems[index].buf[i * core_mp_V2.frame_len]);
...
out:
	if (open != NULL) {
		for (i = 0; i < tItems[index].frame_count; i++) {
			ipio_kfree((void **)&open[i].cap_dac_V2);
			ipio_kfree((void **)&open[i].cap_raw_V2);
...
static int key_test(int index)
{
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		for (j = 0; j < core_mp_V2.key_len; j++)
			tItems[index].buf[j] = key_buf_V2[j];
	}
...
static int mp_get_timing_info(void)
{
	...
	core_mp_V2.isLongV = info[6];

	ipio_info("DDI Mode = %s\n", (core_mp_V2.isLongV ? "Long V" : "Long H"));
...
static int mp_test_data_sort_average(s32 *oringin_data, int index, s32 *avg_result)
{
	...
	u32data_buff = kcalloc(core_mp_V2.frame_len * tItems[index].frame_count, sizeof(s32), GFP_KERNEL);
	u32sum_raw_data = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
	if (ERR_ALLOC_MEM(u32sum_raw_data) || (ERR_ALLOC_MEM(u32data_buff))) {
		ipio_err("Failed to allocate u32sum_raw_data FRAME buffer\n");
		if(u32data_buff != NULL)
			ipio_kfree((void **)&u32data_buff);
		if(u32sum_raw_data != NULL)
			ipio_kfree((void **)&u32sum_raw_data);
		return -ENOMEM;
	}

	for (i = 0; i < core_mp_V2.frame_len * tItems[index].frame_count; i++) {
	...
	if (ipio_debug_level_V2) {
		...
		for (i = 0; i < core_mp_V2.frame_len; i++) {
			for (j = 0 ; j < tItems[index].frame_count ; j++) {
				if ((i < len) || (i >= (core_mp_V2.frame_len-len)))
					pr_cont("%d,", u32data_buff[j * core_mp_V2.frame_len + i]);
			}
			if ((i < len) || (i >= (core_mp_V2.frame_len-len)))
				pr_cont("\n");
		}
	...
	for (i = 0; i < core_mp_V2.frame_len; i++) {
		for (j = 0; j < tItems[index].frame_count-1; j++) {
			for (k = 0; k < (tItems[index].frame_count-1-j); k++) {
				x = i+k*core_mp_V2.frame_len;
				y = i+(k+1)*core_mp_V2.frame_len;
	...
	if (ipio_debug_level_V2) {
		...
		for (i = 0; i < core_mp_V2.frame_len; i++) {
			for (j = u32down_frame; j < tItems[index].frame_count - u32up_frame; j++) {
				if ((i < len) || (i >= (core_mp_V2.frame_len - len)))
					pr_cont("%d,", u32data_buff[i + j * core_mp_V2.frame_len]);
			}
			if ((i < len) || (i >= (core_mp_V2.frame_len-len)))
				pr_cont("\n");
		}
	...
	for (i = 0 ; i < core_mp_V2.frame_len ; i++) {
		u32sum_raw_data[i] = 0;
		for (j = u32down_frame; j < tItems[index].frame_count - u32up_frame; j++)
			u32sum_raw_data[i] += u32data_buff[i + j * core_mp_V2.frame_len];
...
static int mp_comp_result_before_retry(int index)
{
	...
	max_threshold = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
	...
	min_threshold = kcalloc(core_mp_V2.frame_len, sizeof(s32), GFP_KERNEL);
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		if (ERR_ALLOC_MEM(core_mp_V2.rx_delta_buf) || ERR_ALLOC_MEM(core_mp_V2.tx_delta_buf)) {
		...
		for (i = 0; i < core_mp_V2.frame_len; i++) {
			max_threshold[i] = core_mp_V2.TxDeltaMax;
			min_threshold[i] = core_mp_V2.TxDeltaMin;
		}
		mp_compare_cdc_result(index, core_mp_V2.tx_max_buf, max_threshold, min_threshold, &test_result);
		mp_compare_cdc_result(index, core_mp_V2.tx_min_buf, max_threshold, min_threshold, &test_result);

		for (i = 0; i < core_mp_V2.frame_len; i++) {
			max_threshold[i] = core_mp_V2.RxDeltaMax;
			min_threshold[i] = core_mp_V2.RxDeltaMin;
		}

		mp_compare_cdc_result(index, core_mp_V2.rx_max_buf, max_threshold, min_threshold, &test_result);
		mp_compare_cdc_result(index, core_mp_V2.rx_min_buf, max_threshold, min_threshold, &test_result);
	} else {
...
static void mp_show_result(bool lcm_on)
{
	...
	for (i = 0; i < ARRAY_SIZE(tItems); i++) {
		...
		if (strcmp(tItems[i].name, "open_integration_sp") == 0) {
			mp_compare_cdc_show_result(i, frame1_cbk700_V2, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "frame1 cbk700");
			mp_compare_cdc_show_result(i, frame1_cbk250_V2, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "frame1 cbk250");
			mp_compare_cdc_show_result(i, frame1_cbk200_V2, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "frame1 cbk200");
		}

		if (strcmp(tItems[i].name, "open test_c") == 0) {
			mp_compare_cdc_show_result(i, cap_dac_V2, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "CAP_DAC");
			mp_compare_cdc_show_result(i, cap_raw_V2, csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, "CAP_RAW");
		}
		...
		if (tItems[i].catalog == KEY_TEST) {
			for (x = 0; x < core_mp_V2.key_len; x++) {
				DUMP("KEY_%02d ", x);
			...
			for (y = 0; y < core_mp_V2.key_len; y++) {
			...
			for (j = 0; j < core_mp_V2.frame_len; j++) {
				max_threshold[j] = core_mp_V2.TxDeltaMax;
				min_threshold[j] = core_mp_V2.TxDeltaMin;
			}
			mp_compare_cdc_show_result(i, core_mp_V2.tx_max_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold, "TX Max Hold");
			mp_compare_cdc_show_result(i, core_mp_V2.tx_min_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold, "TX Min Hold");

			for (j = 0; j < core_mp_V2.frame_len; j++) {
				max_threshold[j] = core_mp_V2.RxDeltaMax;
				min_threshold[j] = core_mp_V2.RxDeltaMin;
			}
			mp_compare_cdc_show_result(i, core_mp_V2.rx_max_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold, "RX Max Hold");
			mp_compare_cdc_show_result(i, core_mp_V2.rx_min_buf, csv, &csv_len, TYPE_JUGE, max_threshold, min_threshold, "RX Min Hold");
		} else {
			/* general result */
			if (tItems[i].trimmed_mean && tItems[i].catalog != PEAK_TO_PEAK_TEST) {
			...
			for (j = 0; j < get_frame_cont; j++) {
				...
				mp_compare_cdc_show_result(i, &tItems[i].buf[(j*core_mp_V2.frame_len)], csv, &csv_len, TYPE_NO_JUGE, max_threshold, min_threshold, frame_name);
			}
		}
	}
	...
	if (pass_item_count == 0) {
		core_mp_V2.final_result = MP_FAIL;
		if (lcm_on)
		...
		core_mp_V2.final_result = MP_PASS;
		if (lcm_on)
...
static void ilitek_tddi_mp_init_item(void)
{
	...
	memset(&core_mp_V2, 0, sizeof(core_mp_V2));

	core_mp_V2.chip_pid = idev_V2->chip->pid;
	core_mp_V2.fw_ver = idev_V2->chip->fw_ver;
	core_mp_V2.protocol_ver = idev_V2->protocol->ver;
	core_mp_V2.cdc_len = idev_V2->protocol->cdc_len;
	core_mp_V2.no_bk_shift = idev_V2->chip->no_bk_shift;
	core_mp_V2.xch_len = idev_V2->xch_num;
	core_mp_V2.ych_len = idev_V2->ych_num;
	core_mp_V2.frame_len = core_mp_V2.xch_len * core_mp_V2.ych_len;
	core_mp_V2.stx_len = 0;
	core_mp_V2.srx_len = 0;
	core_mp_V2.key_len = 0;
	core_mp_V2.st_len = 0;
	core_mp_V2.tdf = 240;
	core_mp_V2.busy_cdc = INT_CHECK;
	core_mp_V2.retry = false;
	core_mp_V2.final_result = MP_FAIL;

	ipio_info("CHIP = 0x%x\n", core_mp_V2.chip_pid);
	ipio_info("Firmware version = %x\n", core_mp_V2.fw_ver);
	ipio_info("Protocol version = %x\n", core_mp_V2.protocol_ver);
	ipio_info("Read CDC Length = %d\n", core_mp_V2.cdc_len);
	ipio_info("X length = %d, Y length = %d\n", core_mp_V2.xch_len, core_mp_V2.ych_len);
	ipio_info("Frame length = %d\n", core_mp_V2.frame_len);
...
static void mp_test_run(char *item)
{
	...
	if (item == NULL || strncmp(item, " ", strlen(item)) == 0 || core_mp_V2.frame_len == 0) {
		core_mp_V2.final_result = MP_FAIL;
		ipio_err("Invaild string (%s) or frame length (%d)\n", item, core_mp_V2.frame_len);
		return;
	}
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		if (strncmp(item, tItems[i].desp, strlen(item)) == 0) {
			...
			if (strcmp(item, "tx/rx delta") == 0) {
				...
				core_mp_V2.TxDeltaMax = katoi_V2(str);
				parser_get_int_data(item, "tx min", str);
				core_mp_V2.TxDeltaMin = katoi_V2(str);
				parser_get_int_data(item, "rx max", str);
				core_mp_V2.RxDeltaMax = katoi_V2(str);
				parser_get_int_data(item, "rx min", str);
				core_mp_V2.RxDeltaMin = katoi_V2(str);
				ipio_debug("%s: Tx Max = %d, Tx Min = %d, Rx Max = %d,  Rx Min = %d\n",
						tItems[i].desp, core_mp_V2.TxDeltaMax, core_mp_V2.TxDeltaMin,
						core_mp_V2.RxDeltaMax, core_mp_V2.RxDeltaMin);
			} else {
				parser_get_int_data(item, "max", str);
				tItems[i].max = katoi_V2(str);
				parser_get_int_data(item, "min", str);
				tItems[i].min = katoi_V2(str);
			}
...
static void mp_test_free(void)
{
	...
	ipio_kfree((void **)&frame1_cbk700_V2);
	ipio_kfree((void **)&frame1_cbk250_V2);
	ipio_kfree((void **)&frame1_cbk200_V2);
	ipio_kfree((void **)&frame_buf_V2);
	ipio_kfree((void **)&key_buf_V2);
}
...
int ilitek_tddi_mp_test_main_V2(char *apk, bool lcm_on)
{
	int ret = 0;
	char str[128] = {0}, ver[128] = {0};

	ilitek_ini_file_data_V2 = (struct ini_file_data *)vmalloc(sizeof(struct ini_file_data) * PARSER_MAX_KEY_NUM);
	if (ERR_ALLOC_MEM(ilitek_ini_file_data_V2)) {
		ipio_info("Failed to malloc ilitek_ini_file_data_V2\n");
		goto out;
	}
	...
	if (ret < 0) {
		ret = ilitek_tddi_mp_ini_parser("/sdcard/mp.ini");
		if (ret < 0) {
	...
	if ((str2hex_V2(ver)) != (core_mp_V2.protocol_ver >> 8)) {
	...
	if (idev_V2->protocol->ver >= PROTOCOL_VER_540) {
		if (lcm_on) {
...
out:
	ipio_vfree((void **)&ilitek_ini_file_data_V2);
	return ret;
};
