...
int bbk_chipone_get_fw_debug_info(unsigned char *buf)
{
    ...
    ret = cts_fw_reg_readb(cts_dev, 0x8015, &scan_freq);
    if (ret) {
...
ssize_t bbk_chipone_readUdd(struct vts_device *vtsdev, u8 *buf, size_t nbytes)
{
	...
	vts_property_get(vtsdev, VTS_PROPERTY_NO_FLASH_UNIQUE_CODE, &new_unique);
	if (0 == new_unique) {
		VTI("chipone not support rom read");
		return 0;
	}
...
int bbk_chipone_getUdd_size(struct vts_device *vtsdev, u32 *size)
{
	...
	vts_property_get(vtsdev, VTS_PROPERTY_NO_FLASH_UNIQUE_CODE, &new_unique);
	if (0 == new_unique) {
		VTI("chipone not support get rom size");
		*size = 0;
	} else {
		*size = 15;
	}
