...
static ssize_t ilitek_proc_vivo_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if (strcmp(cmd, "rawordiff") == 0) {
		memset(debug, 0x0, 32 * 18 * sizeof(int));
		ret = bbk_ili_get_rawordiff_data(data[1], debug);
		ipio_info("Get RAW/Diff data %s\n", (ret < 0) ? "FAIL" : "DONE");
		ret = ipio_debug_level;
		ipio_debug_level = DEBUG_ALL;
		ilitek_dump_data(debug, 8, 32 * 18, 0, "vivo debug");
		ipio_debug_level = ret;
	} else if (strcmp(cmd, "fwupdate") == 0) {
		if (request_firmware(&fw, UPDATE_REQUEST_FW_PATH, idev->dev) < 0) {
			ipio_err("Rquest firmware failed\n");
			goto out;
		}
		mutex_lock(&idev->touch_mutex);
		ret = bbk_ili_fw_update(fw);
		mutex_unlock(&idev->touch_mutex);
		ipio_info("fw upgrade %s\n", (ret < 0) ? "FAIL" : "DONE");
		release_firmware(fw);
	} else if (strcmp(cmd, "idlectrl") == 0) {
		ret = iliIdleEnableOrDisable(data[1]);
		ipio_info("set idle %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "readudd") == 0) {
		bbk_ili_readUdd(NULL);
	} else if (strcmp(cmd, "writeudd") == 0) {
		bbk_ili_writeUdd(NULL);
	} else if (strcmp(cmd, "modechange") == 0) {
		ret = bbk_ili_mode_change(data[1]);
		ipio_info("switch mode %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "getfwver") == 0) {
		ret = bbk_ili_get_fw_version(data[1]);
		ipio_info("get fw version %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "getgesture") == 0) {
		ret = bbk_ili_gesture_point_get(gesture);
		ipio_info("Get gesture len (%d), %s\n", ret, (ret < 0) ? "FAIL" : "DONE");
		ret = ipio_debug_level;
		ipio_debug_level = DEBUG_ALL;
		ilitek_dump_data(gesture, 8, sizeof(gesture), 0, "vivo gesture after");
		ipio_debug_level = ret;
	} else if (strcmp(cmd, "setcharger") == 0) {
		ret = bbk_ili_set_charger_bit(data[1]);
		ipio_info("set charger %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "readcharger") == 0) {
		bbk_ili_read_charger_bit();
	} else if (strcmp(cmd, "setedge") == 0) {
		ret = iliSetEdgeRestainSwitch(data[1]);
		ipio_info("set edge palm %s\n", (ret < 0) ? "Fail" : "DONE");
	} else if (strcmp(cmd, "getfwdebuginfo") == 0) {
		unsigned char *str = NULL;
		str = kzalloc(PAGE_SIZE, GFP_KERNEL);
		ret = bbk_ili_get_fw_debug_info(str);
...
struct file_operations proc_vivo_fops = {
	.write = ilitek_proc_vivo_write,
};

struct file_operations proc_vivo_mp_test_fops = {
	.read = ilitek_node_mp_lcm_on_test_read,
};

struct file_operations proc_ver_info_ops = {
...
proc_node_t proc_table[] = {
	...
	{"vivo_test", NULL, &proc_vivo_fops, false},
	{"mp_test", NULL, &proc_vivo_mp_test_fops, false},
	{"ver_info", NULL, &proc_ver_info_ops, false},
