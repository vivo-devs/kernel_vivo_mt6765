...
static int ilitek_ts_dev_suspend(struct device *dev)
{
	//struct ilitek_tddi_dev *idev = dev_get_drvdata(dev);
	idev->ili_suspended = true;
	return 0;
}

static int ilitek_ts_dev_resume(struct device *dev)
{
	//struct ilitek_tddi_dev *idev = dev_get_drvdata(dev);	
...
static const struct dev_pm_ops ilitek_ts_pm = {
	.suspend = ilitek_ts_dev_suspend,
	.resume = ilitek_ts_dev_resume,
};	
...
int ilitek_tddi_interface_dev_init(struct ilitek_hwif_info *hwif)
{
	...
	info->bus_driver.driver.pm = &ilitek_ts_pm;
