					...
					struct kobj_attribute *attr, char *buf)
{
	...
	case VTS_TOUCH_INFO_COLLECT_DUMP:
		ret = snprintf(buf, 255, "statusCount:%02x,%02x,%02x,%02x\nstatusBits:%08x,%08x,%08x,%08x\n", vivo_ts_data->statisticsCount[0], vivo_ts_data->statisticsCount[1],
					...
					struct kobj_attribute *attr, char *buf)
{
	...
	if (vivoTsGetVtsData()->isProbeComplete == 0) {		
		VTI("probe is not complete\n");
	...
	VTI("vivo_ts_data->noFlash %d", vivo_ts_data->noFlash);
	if (!vivo_ts_data->noFlash) {
		/* ID is invalid */
		for (i = 0; i < 15; i++) {
		...
		VTI("0-flash read from firware");
		return snprintf(buf, 255, "%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x\n",
			read_config_buf[0],
			read_config_buf[1],
			read_config_buf[2],
			read_config_buf[3],
