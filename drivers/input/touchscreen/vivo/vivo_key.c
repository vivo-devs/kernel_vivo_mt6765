...
static void menu_release_timer_handler(struct timer_list * data)
{
...
static void back_release_timer_handler(struct timer_list * data)
{
	struct vivo_key *pvivo_key = (struct vivo_key *)data;
	if (!pvivo_key) {
		VIVO_KEY_LOG_ERR("The up_slide pointer is NULL\n");
...
static void home_release_timer_handler(struct timer_list * data)
{
	struct vivo_key *pvivo_key = (struct vivo_key *)data;
	if (!pvivo_key) {
		VIVO_KEY_LOG_ERR("The up_slide pointer is NULL\n");
...
int register_vivo_key(struct vivo_key *pvivo_key)
{
	...
	timer_setup(&pvivo_key->menu_release_timer,
				menu_release_timer_handler, 0);
				//NULL, 0);
	INIT_WORK(&pvivo_key->menu_release_work, menu_release_work_handler);

	timer_setup(&pvivo_key->back_release_timer,
				back_release_timer_handler, 0);
	INIT_WORK(&pvivo_key->back_release_work, back_release_work_handler);
#if defined(HOME_KEY_SUPPORTED)
	timer_setup(&pvivo_key->home_release_timer,
				home_release_timer_handler, 0);
