...
const struct firmware *fw_entry_1818C = NULL;
 ...
 *******************************************************/
int32_t update_firmware_request_1818C(char *filename)
{
	int32_t ret = 0;
	int size = 0;
...
if (0) {
	...
	/*ret = request_firmware(&fw_entry_1818C, filename, &ts_1818C->client->dev); */
	/*if (ret) { */
	...
	fw_entry_1818C = &nt_fw;

	/* check bin file size (116kb) */
	if (fw_entry_1818C->size != FW_BIN_SIZE) {
		NVT_ERR("bin file size not match. (%zu)\n", fw_entry_1818C->size);
		return -EINVAL;
	}

	/* check if FW version add FW version bar equals 0xFF */
	if (*(fw_entry_1818C->data + FW_BIN_VER_OFFSET) + *(fw_entry_1818C->data + FW_BIN_VER_BAR_OFFSET) != 0xFF) {
		VTE("bin file FW_VER + FW_VER_BAR should be 0xFF!");
		VTE("FW_VER=0x%02X, FW_VER_BAR=0x%02X", *(fw_entry_1818C->data + FW_BIN_VER_OFFSET), *(fw_entry_1818C->data + FW_BIN_VER_BAR_OFFSET));
		return -EINVAL;
	}
...
return:
	n.a.
 *******************************************************/
void update_firmware_release_1818C(void)
{
	if (fw_entry_1818C) {
		release_firmware(fw_entry_1818C);
	}
	fw_entry_1818C = NULL;
}
...
int32_t Check_FW_Ver_1818C(void)
{
	uint8_t buf[16] = {0};
	int32_t ret = 0;
	...
	buf[1] = 0x00;
	buf[2] = 0x00;
	ret = CTP_I2C_READ_1818C(ts_1818C->client, I2C_BLDR_Address, buf, 3);
	if (ret < 0) {
	...
	NVT_LOG("Bin FW Ver = 0x%02X, FW ver Bar = 0x%02X\n",
			fw_entry_1818C->data[FW_BIN_VER_OFFSET], fw_entry_1818C->data[FW_BIN_VER_BAR_OFFSET]);
	...
	if (buf[1] != fw_entry_1818C->data[FW_BIN_VER_OFFSET])
		return 0;
	else
		return 1;
}

int get_chip_FW_Ver_1818C(void)
{
	uint8_t buf[16] = {0};
	int32_t ret = 0;
...
int nt_get_ic_fw_version_1818C(void)
{
 ...
 *******************************************************/
int32_t Check_CheckSum_1818C(void)
{
	uint8_t buf[64] = {0};
	uint32_t XDATA_Addr = ts_1818C->mmap->READ_FLASH_CHECKSUM_ADDR;
	int32_t ret = 0;
	int32_t i = 0;
	int32_t k = 0;
	...
	fw_bin_size = fw_entry_1818C->size;
	...
	for (i = 0; i < BLOCK_64KB_NUM; i++) {
		if (fw_bin_size > (i * SIZE_64KB)) {
			...
			for (k = 0; k < len_in_blk; k++) {
				WR_Filechksum[i] += fw_entry_1818C->data[k + i * SIZE_64KB];
			}
			...
			ret = CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_HW_Address, buf, 7);
			if (ret < 0) {
...
return:
	Executive outcomes. 0-- - succeed. negative-- - failed.
 *******************************************************/
int32_t Init_BootLoader_1818C(void)
{
	uint8_t buf[64] = {0};
	int32_t ret = 0;
	int32_t retry = 0;
...
return:
	Executive outcomes. 0-- - succeed. negative-- - failed.
 *******************************************************/
int32_t Erase_Flash_1818C(void)
{
	uint8_t buf[64] = {0};
	int32_t ret = 0;
	int32_t count = 0;
	...
	if (fw_entry_1818C->size % FLASH_SECTOR_SIZE)
		count = fw_entry_1818C->size / FLASH_SECTOR_SIZE + 1;
	else
		count = fw_entry_1818C->size / FLASH_SECTOR_SIZE;
...
return:
	Executive outcomes. 0-- - succeed. negative-- - failed.
 *******************************************************/
int32_t Write_Flash_1818C(void)
{
	uint8_t buf[64] = {0};
	...
	if (fw_entry_1818C->size % 256)
		count = fw_entry_1818C->size / 256 + 1;
	else
		count = fw_entry_1818C->size / 256;
	...
	for (i = 0; i < count; i++) {
		...
		for (j = 0; j < min(fw_entry_1818C->size - i * 256, (size_t)256); j += 32) {
		...
		if (fw_entry_1818C->size - Flash_Address >= 256)
			tmpvalue = (Flash_Address >> 16) + ((Flash_Address >> 8) & 0xFF) + (Flash_Address & 0xFF) + 0x00 + (255);
		else
			tmpvalue = (Flash_Address >> 16) + ((Flash_Address >> 8) & 0xFF) + (Flash_Address & 0xFF) + 0x00 + (fw_entry_1818C->size - Flash_Address - 1);

		for (k = 0; k < min(fw_entry_1818C->size - Flash_Address, (size_t)256); k++)
			tmpvalue += fw_entry_1818C->data[Flash_Address + k];
...
return:
	Executive outcomes. 0-- - succeed. negative-- - failed.
 *******************************************************/
int32_t Verify_Flash_1818C(void)
{
	uint8_t buf[64] = {0};
	uint32_t XDATA_Addr = ts_1818C->mmap->READ_FLASH_CHECKSUM_ADDR;
	int32_t ret = 0;
...
return:
	Executive outcomes. 0-- - succeed. negative-- - failed.
 *******************************************************/
int32_t Update_Firmware_1818C(void)
{
	int32_t ret = 0;

	nvt_stop_crc_reboot_1818C();
	/* Step 1 : initial bootloader */
	ret = Init_BootLoader_1818C();
	if (ret) {
		return ret;
	}
	...
	/* Step 3 : Erase */
	ret = Erase_Flash_1818C();
	if (ret) {
		return ret;
	}

	/* Step 4 : Program */
	ret = Write_Flash_1818C();
	if (ret) {
		return ret;
	}

	/* Step 5 : Verify */
	ret = Verify_Flash_1818C();
	if (ret) {
		return ret;
	}
...
return:
	n.a.
 *******************************************************/
void Boot_Update_Firmware_1818C_1818C(struct work_struct *work)
{
	int32_t ret = 0;
	...
	ret = update_firmware_request_1818C(firmware_name);
	if (ret) {
		VTE("update_firmware_request_1818C failed. (%d)", ret);
		goto out;
	}
	...
	nvt_sw_reset_idle_1818C();
	ret = Check_CheckSum_1818C();
	if (ret < 0) {	/* read firmware checksum failed */
		VTI("read firmware checksum failed");
		Update_Firmware_1818C();
	} else if ((ret == 0) || (Check_FW_Ver_1818C() == 0)) {	/* (fw checksum not match) || (bin fw version != ic fw version) */
...
out:
	...
	ic_fw_version = get_chip_FW_Ver_1818C();
}

int hand_update_firmware_1818C(const struct firmware *firmware)
{
	const struct firmware *tmp_fw = fw_entry_1818C;
	...
	fw_entry_1818C = firmware;
	nvt_sw_reset_idle_1818C();

	VTE("firmware version not match");
	Update_Firmware_1818C();

	fw_entry_1818C = tmp_fw;
	mutex_unlock(&ts_1818C->lock);
