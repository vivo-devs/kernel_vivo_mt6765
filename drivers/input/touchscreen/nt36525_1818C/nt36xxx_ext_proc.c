...
static int32_t xdata_q[2048] = {0};
	...
	int32_t xdata_custom_1818C[2048] = {0};
#endif
...
return:
	n.a.
 *******************************************************/
void nvt_change_mode_1818C(uint8_t mode)
{
	uint8_t buf[8] = {0};
 ...
 *******************************************************/
uint8_t nvt_get_fw_pipe_1818C(void)
{
	uint8_t buf[8] = {0};
...
return:
	n.a.
 *******************************************************/
void nvt_read_mdata_1818C(uint32_t xdata_addr, uint32_t xdata_btn_addr)
{
	int32_t i = 0;
	int32_t j = 0;
	int32_t k = 0;
	...
	data_len = ts_1818C->x_num * ts_1818C->y_num * 2;
	...
	for (i = 0; i < ((dummy_len + data_len) / XDATA_SECTOR_SIZE); i++) {
		...
		for (j = 0; j < (XDATA_SECTOR_SIZE / I2C_TANSFER_LENGTH); j++) {
			...
			CTP_I2C_READ_1818C(ts_1818C->client, I2C_FW_Address, buf, I2C_TANSFER_LENGTH + 1);
	...
	CTP_I2C_READ_1818C(ts_1818C->client, I2C_FW_Address, buf, (TOUCH_KEY_NUM * 2 + 1));

	/*-- -2bytes - to -1data-- - */
	for (i = 0; i < TOUCH_KEY_NUM; i++) {
		xdata[ts_1818C->x_num * ts_1818C->y_num + i] = (int16_t)(buf[1 + i * 2] + 256 * buf[1 + i * 2 + 1]);
	}
#endif
...
return:
	n.a.
 *******************************************************/
void nvt_read_mdata_1818C_rss_1818C(uint32_t xdata_i_addr, uint32_t xdata_q_addr, uint32_t xdata_btn_i_addr, uint32_t xdata_btn_q_addr)
{
	int i = 0;

	nvt_read_mdata_1818C(xdata_i_addr, xdata_btn_i_addr);
	memcpy(xdata_i, xdata, ((ts_1818C->x_num * ts_1818C->y_num + TOUCH_KEY_NUM) * sizeof(int32_t)));

	nvt_read_mdata_1818C(xdata_q_addr, xdata_btn_q_addr);
	memcpy(xdata_q, xdata, ((ts_1818C->x_num * ts_1818C->y_num + TOUCH_KEY_NUM) * sizeof(int32_t)));

	for (i = 0; i < (ts_1818C->x_num * ts_1818C->y_num + TOUCH_KEY_NUM); i++) {
...
return:
    n.a.
 *******************************************************/
void nvt_get_mdata_1818C(int32_t *buf, uint8_t *m_x_num, uint8_t *m_y_num)
{
    *m_x_num = ts_1818C->x_num;
    *m_y_num = ts_1818C->y_num;
    memcpy(buf, xdata, ((ts_1818C->x_num * ts_1818C->y_num + TOUCH_KEY_NUM) * sizeof(int32_t)));
}
...
{
	seq_printf(m, "fw_ver=%d, x_num=%d, y_num=%d, button_num=%d\n", ts_1818C->fw_ver, ts_1818C->x_num, ts_1818C->y_num, ts_1818C->max_button_num);
	return 0;
}
...
static int32_t c_show(struct seq_file *m, void *v)
{
	...
	for (i = 0; i < ts_1818C->y_num; i++) {
		for (j = 0; j < ts_1818C->x_num; j++) {
			seq_printf(m, "%5d, ", xdata[i * ts_1818C->x_num + j]);
		}
		seq_puts(m, "\n");
	}
...
#if TOUCH_KEY_NUM > 0
	for (i = 0; i < TOUCH_KEY_NUM; i++) {
		seq_printf(m, "%5d, ", xdata[ts_1818C->x_num * ts_1818C->y_num + i]);
	}
	seq_puts(m, "\n");
#endif
...
const struct seq_operations nvt_fw_version_seq_ops_1818C = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
...
const struct seq_operations nvt_seq_ops_1818C = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
	.show   = c_show
...
{
	if (mutex_lock_interruptible(&ts_1818C->lock)) {
		return -ERESTARTSYS;
	}
	...
	return seq_open(file, &nvt_fw_version_seq_ops_1818C);
}
...
static int32_t nvt_baseline_open(struct inode *inode, struct file *file)
{
	...
	if (nvt_clear_fw_status_1818C()) {
		mutex_unlock(&ts_1818C->lock);
		return -EAGAIN;
	}

	nvt_change_mode_1818C(TEST_MODE_2);

	if (nvt_check_fw_status_1818C()) {
		mutex_unlock(&ts_1818C->lock);
		return -EAGAIN;
	}
	...
	if (ts_1818C->carrier_system) {
		nvt_read_mdata_1818C_rss_1818C(ts_1818C->mmap->BASELINE_ADDR, ts_1818C->mmap->BASELINE_Q_ADDR,
				ts_1818C->mmap->BASELINE_BTN_ADDR, ts_1818C->mmap->BASELINE_BTN_Q_ADDR);
	} else {
		nvt_read_mdata_1818C(ts_1818C->mmap->BASELINE_ADDR, ts_1818C->mmap->BASELINE_BTN_ADDR);
	}

	nvt_change_mode_1818C(NORMAL_MODE);

	mutex_unlock(&ts_1818C->lock);

	NVT_LOG("--\n");

	return seq_open(file, &nvt_seq_ops_1818C);
}
...
static int32_t nvt_raw_open(struct inode *inode, struct file *file)
{
	...
	if (ts_1818C->carrier_system) {
		if (nvt_get_fw_pipe_1818C() == 0)
			nvt_read_mdata_1818C_rss_1818C(ts_1818C->mmap->RAW_PIPE0_ADDR, ts_1818C->mmap->RAW_PIPE0_Q_ADDR,
				ts_1818C->mmap->RAW_BTN_PIPE0_ADDR, ts_1818C->mmap->RAW_BTN_PIPE0_Q_ADDR);
		else
			nvt_read_mdata_1818C_rss_1818C(ts_1818C->mmap->RAW_PIPE1_ADDR, ts_1818C->mmap->RAW_PIPE1_Q_ADDR,
				ts_1818C->mmap->RAW_BTN_PIPE1_ADDR, ts_1818C->mmap->RAW_BTN_PIPE1_Q_ADDR);
	} else {
		if (nvt_get_fw_pipe_1818C() == 0)
			nvt_read_mdata_1818C(ts_1818C->mmap->RAW_PIPE0_ADDR, ts_1818C->mmap->RAW_BTN_PIPE0_ADDR);
		else
			nvt_read_mdata_1818C(ts_1818C->mmap->RAW_PIPE1_ADDR, ts_1818C->mmap->RAW_BTN_PIPE1_ADDR);
	}
...
static int32_t nvt_diff_open(struct inode *inode, struct file *file)
{
	...
	if (ts_1818C->carrier_system) {
		if (nvt_get_fw_pipe_1818C() == 0)
			nvt_read_mdata_1818C_rss_1818C(ts_1818C->mmap->DIFF_PIPE0_ADDR, ts_1818C->mmap->DIFF_PIPE0_Q_ADDR,
				ts_1818C->mmap->DIFF_BTN_PIPE0_ADDR, ts_1818C->mmap->DIFF_BTN_PIPE0_Q_ADDR);
		else
			nvt_read_mdata_1818C_rss_1818C(ts_1818C->mmap->DIFF_PIPE1_ADDR, ts_1818C->mmap->DIFF_PIPE1_Q_ADDR,
				ts_1818C->mmap->DIFF_BTN_PIPE1_ADDR, ts_1818C->mmap->DIFF_BTN_PIPE1_Q_ADDR);
	} else {
		if (nvt_get_fw_pipe_1818C() == 0)
			nvt_read_mdata_1818C(ts_1818C->mmap->DIFF_PIPE0_ADDR, ts_1818C->mmap->DIFF_BTN_PIPE0_ADDR);
		else
			nvt_read_mdata_1818C(ts_1818C->mmap->DIFF_PIPE1_ADDR, ts_1818C->mmap->DIFF_BTN_PIPE1_ADDR);
	}
...
static int32_t nvtc_FlashID_open(struct inode *inode, struct file *file)
{
	...
	mutex_unlock(&ts_1818C->lock);
	{
		...
		i32ret = bbk_nt_sensor_test_1818C(testBuf, NULL, -1);/*<-Watch!Contain Mutex Lock/unLock*/
 ...
 *******************************************************/
int32_t nvt_extra_proc_init_1818C(void)
{
...
extern int nvt_sw_reset_idle_1818C(void);
extern int32_t Update_Firmware_1818C(void);
extern int32_t Resume_PD_1818C(void);
extern void nvt_bootloader_reset_1818C(void);
extern int32_t Init_BootLoader_1818C(void);
...
uint8_t u8A_IDread_1818C[FLASH_ID_READ_LEN + 1] = {"999999999"};
uint8_t u8A_IDwrite_1818C[64]   = {"1234567890*ABCDEFGHJ123456789012"};/*len is 32 */

int32_t nvtc_Erase_Flash_1818C_Sector(void)
{
	uint8_t buf[64] = {0};
	int32_t ret = 0;
	/*int32_t count = 0; */
	...
	while (1) {
		...
		buf[0] = 0x00;
		buf[1] = 0x00;
		ret = CTP_I2C_READ_1818C(ts_1818C->client, I2C_HW_Address, buf, 2);
		if (ret < 0) {
	...
	buf[1] = 0x01;
	buf[2] = 0x00;
	ret = CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_HW_Address, buf, 3);
	if (ret < 0) {
	...
	while (1) {
		...
		buf[1] = 0x00;
		buf[2] = 0x00;
		ret = CTP_I2C_READ_1818C(ts_1818C->client, I2C_HW_Address, buf, 3);
		if (ret < 0) {
	...
	i = SectorSpecify;/*31[20170301, jx] < Specify 0x1F000~0x1FFF > */
	{
		...
		ret = CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_HW_Address, buf, 5);
		if (ret < 0) {
...
int32_t nvtc_Write_Flash_ID_1818C(uint8_t *u8Am_IDdata)
{

/*	uint8_t u8StrBuff[32 + 1]; */

	uint8_t buf[64] = {0};
	uint32_t XDATA_Addr = ts_1818C->mmap->RW_FLASH_DATA_ADDR;
...
/*	int32_t count = 0; */
	...
	i = SectorSpecify;
	{
		...
		j = 0;
		{
			...
			for (k = 0; k < 32; k++) {
				/*buf[1 + k] = fw_entry_1818C->data[Flash_Address + j + k]; */
				buf[1 + k] = u8Am_IDdata[k];
			}
			ret = CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_BLDR_Address, buf, 33);
			if (ret < 0) {
		...
		buf[4] = (Flash_Address & 0xFF);
		buf[5] = 0x00;
	  /*buf[6] = min(fw_entry_1818C->size - Flash_Address, (size_t)256) - 1; */
	    buf[6] = sizeMax - 1;
		buf[7] = tmpvalue;
		ret = CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_HW_Address, buf, 8);
		if (ret < 0) {
...
int32_t nvtc_write_projectid_1818C(uint8_t *u8Am_IDdata)
{
	uint8_t buf[64] = {0};
	int retval = 0;/*NO_ERR; */

	/*-- - stop fw-- - */
	nvt_sw_reset_idle_1818C();
	/*-------------------------- - */

	/* Step 1 : initial bootloader */
	retval = Init_BootLoader_1818C();
	if (retval) {
		return retval;
	}

	/* Step 2 : Resume PD */
	retval = Resume_PD_1818C();
	if (retval) {
		return retval;
	}
	...
	buf[0] = 0x00;
	buf[1] = 0x35;
	retval = CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_HW_Address, buf, 2);
	if (retval < 0) {
	...
	/* Step 3 -2 : Erase */
	retval = nvtc_Erase_Flash_1818C_Sector();
	if (retval) {
	...
	/* Step 4 : Program */
	nvtc_Write_Flash_ID_1818C(u8Am_IDdata);
	/*-- - reset ic to run fw-- - */
...
int32_t nvtc_read_projectid_1818C(uint8_t *u8Am_IDread)
{
	uint8_t buf[64] = {0};
	uint32_t XDATA_Addr = ts_1818C->mmap->RW_FLASH_DATA_ADDR;
	int retval = 0;//NO_ERR;
...
/*	buf[3] = 0xF0; */
	...
	buf[5] = 0x00;
	buf[6] = 0x20;
	retval = CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_HW_Address, buf, 7);
	if (retval < 0) {
	...
	buf[0] = 0x00;
	buf[1] = 0x00;
	retval = CTP_I2C_READ_1818C(ts_1818C->client, I2C_HW_Address, buf, 2);
	if (retval < 0) {
	...
	buf[1] = XDATA_Addr >> 16;
	buf[2] = (XDATA_Addr >> 8) & 0xFF;
	retval = CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_HW_Address, buf, 3);
	if (retval < 0) {
	...
	retval = CTP_I2C_READ_1818C(ts_1818C->client, I2C_BLDR_Address, buf, 33);
	if (retval < 0) {
		NVT_ERR("%s: Check 0xAA (Read Command) error!!(%d)\n", __func__, retval);
		return retval;
	}
...
extern uint8_t bTouchIsAwake_1818C;
int8_t nvt_customizeCmd_WaitSet_1818C(uint8_t u8WaitAddr, RST_COMPLETE_STATE_1818C check_reset_state, uint8_t u8Cmd)
{
	uint8_t buf[8] = {0};
	int32_t i = 0;
	int32_t retry_max = (check_reset_state == RESET_STATE_INIT) ? 10 : 50;
	NVT_LOG("++parameter(0x%02X,0x%02X,0x%02X)\n", u8WaitAddr, check_reset_state, u8Cmd);
	mutex_lock(&ts_1818C->lock);
	VTI("start");
	for (i = 0; i < retry_max; i++) {
		...
		buf[0] = u8WaitAddr;
		buf[1] = 0x00;
		CTP_I2C_READ_1818C(ts_1818C->client, I2C_FW_Address, buf, (2 + 1));
	  ...
	  //if ((buf[1] & 0xF0) == u8WaitStatus)
		...
		if (buf[1] >= check_reset_state) {   //[20170815,jx]set      >= u8WaitStatus , is for A->B->C mode
			break;
		}
		if (i >= retry_max) {
...
int tpd_usb_plugin_1818C(int plugin)
{
	int ret = -1;
	...
	case USB_PLUGOUT:
		...
		if (ret < 0) {
			NVT_LOG("tpd_usb_plugin_1818C 0x%02X cmd failed.\n", EVENTBUFFER_STATUS_OFF);
		}
		break;
...
int vivo_nt_readUdd_1818C(unsigned char *udd)
{
	int32_t i32ret = 0;
	...
	nvt_esd_check_enable_1818C(false);
#endif
	i32ret = nvtc_read_projectid_1818C(udd);

	mutex_unlock(&ts_1818C->lock);
	NVT_LOG("i32ret=[%d]\n", i32ret);/* / jx */
	return	i32ret;
}
int vivo_nt_writeUdd_1818C(unsigned char *udd)
{
	int32_t i32ret = 0;
	...
	nvtc_write_projectid_1818C(udd);
...
int32_t i32chargerFlag_1818C = 0;

int vivo_nt_set_charger_bit_1818C(int state)
{
	int ret = 0;
...
/*	if (vivoTsGetVtsData()->hasLcdShutoff) {
	...
	ret = tpd_usb_plugin_1818C(state);
	if (ret < 0) {
...
#if(1)
int vivo_nt_read_charger_bit_1818C(void)
{	
	uint8_t buf[8] = {0};
	buf[0] = 0xFF;
	buf[1] = (ts_1818C->mmap->EVENT_BUF_ADDR >> 16) & 0xFF;
...
{
	int32_t i32ret = 0;
	if ((i32chargerFlag_1818C == USB_PLUGOUT) || (i32chargerFlag_1818C == USB_PLUGIN)) {
		i32ret = i32chargerFlag_1818C;
	} else {
		i32ret = (-1);
	}
		NVT_LOG("i32ret=[%d]\n", i32ret);/* / jx */
...
#endif
int vivo_nt_idle_ctl_1818C(int state)
{
	int32_t i32ret = 0;
...
int vivo_nt_set_edge_restain_switch_1818C(int on)
{
	int32_t ret = 0;
...
int vivo_nt_get_rawordiff_data_1818C(int which, int *data)
{
	int32_t i32ret = 0;
	int x = 0, y = 0;
	VTI("enter");
	...
	if (which == 0) {/*0, raw */
		...
		VTI("(x,y)=(%d,%d)", ts_1818C->x_num, ts_1818C->y_num);	/* / x = 18 y = 30 */

if (i32ret != (-EINVAL)) {
	/*ts_1818C->y_num = ts_1818C->y_num -2; */

	for (y = 0; y < ts_1818C->y_num; y++) {
		printk("VIVO_TS");
		for (x = 0; x < ts_1818C->x_num; x++) {
			 *(data + x * ts_1818C->y_num + y) = *(xdata + y * ts_1818C->x_num + x);
			printk("%6d ", *(xdata + y * ts_1818C->x_num + x));
		}
		printk("\n");
	}
}
/*	if (i32ret != (-EINVAL)) { */
	/*	memcpy(data, xdata, ((ts_1818C->x_num * ts_1818C->y_num + TOUCH_KEY_NUM) * sizeof(int32_t)));	 */
/*	} */
...
static int8_t nvt_customizeExtCmd(uint8_t u8Cmd, uint8_t u8subCmd, uint8_t u8Param)
{
	uint8_t buf[8] = {0};
	uint8_t retry = 0;
	int8_t ret = 0;
	...
	//NVT_LOG("++\n");
	mutex_lock(&ts_1818C->lock);
	NVT_LOG("++ Cmd=0x%02X, subcmd=0x%02X,param=0x%02X \n",u8Cmd,u8subCmd,u8Param);//[20171101]Modi
	for (retry = 0; retry < 20; retry++) {
		/*-- - set xdata index to EVENT BUF ADDR-- - */
		buf[0] = 0xFF;
		buf[1] = (ts_1818C->mmap->EVENT_BUF_ADDR >> 16) & 0xFF;
	...
	if (unlikely(retry == 20)) {
		...
		buf[1] = 0xFF;
		CTP_I2C_READ_1818C(ts_1818C->client, I2C_FW_Address, buf, 2);
		NVT_LOG("read feed back :0x%02X",buf[1]);
		buf[1] &= 0x08;
		if (buf[1] == u8Param)
			NVT_LOG("read feed back corect");
		else
			NVT_LOG("read feed back fail");
	}
	mutex_unlock(&ts_1818C->lock);
	NVT_LOG("--\n");
	return ret;
...
static int bbk_xxx_game_mode_ctl(u8 gamemode)
{
	int ret;
	if(gamemode)
	ret = nvt_customizeExtCmd(0x7F,0x02,0x01);
	else
	ret = nvt_customizeExtCmd(0x7F,0x02,0x00);
	if(ret < 0){
		NVT_ERR("nt gamemode set fail");
	}
	return ret;
}
int bbk_xxx_process_by_package_1818C(unsigned char *package_name)
{
	...
	VTI("package:%s",package_name);
	if (strcmp(package_name, "GAME_OUT") == 0) {
		VTI("GAME_OUT");
