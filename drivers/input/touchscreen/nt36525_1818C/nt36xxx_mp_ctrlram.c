...
extern void nvt_change_mode_1818C(uint8_t mode);
extern uint8_t nvt_get_fw_pipe_1818C(void);
extern void nvt_read_mdata_1818C(uint32_t xdata_addr, uint32_t xdata_btn_addr);
extern void nvt_get_mdata_1818C(int32_t *buf, uint8_t *m_x_num, uint8_t *m_y_num);
void nvt_mp_parse_dt_1818C(struct device_node *root, const char *node_compatible);
...
static int32_t nvt_read_baseline(int32_t *xdata)
{
	...
	nvt_get_mdata_1818C(xdata, &x_num, &y_num);
...
static void nvt_enable_noise_collect(int32_t frame_num)
{
	...
	buf[3] = frame_num;
	buf[4] = 0x00;
	CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_FW_Address, buf, 5);
}
...
static int32_t nvt_read_fw_open(int32_t *xdata)
{
	...
	if (nvt_get_fw_pipe_1818C() == 0)
		raw_pipe_addr = ts_1818C->mmap->RAW_PIPE0_ADDR;
	else
		raw_pipe_addr = ts_1818C->mmap->RAW_PIPE1_ADDR;
	...
	for (y = 0; y < IC_Y_CFG_SIZE; y++) {
		...
		CTP_I2C_READ_1818C(ts_1818C->client, I2C_FW_Address, buf, IC_X_CFG_SIZE * 2 + 1);
		memcpy(rawdata_buf + y * IC_X_CFG_SIZE * 2, buf + 1, IC_X_CFG_SIZE * 2);
	}
#if TOUCH_KEY_NUM > 0
	if (nvt_get_fw_pipe_1818C() == 0)
		raw_btn_pipe_addr = ts_1818C->mmap->RAW_BTN_PIPE0_ADDR;
	else
		raw_btn_pipe_addr = ts_1818C->mmap->RAW_BTN_PIPE1_ADDR;
	...
	CTP_I2C_READ_1818C(ts_1818C->client, I2C_FW_Address, buf, IC_KEY_CFG_SIZE * 2 + 1);
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	for (y = 0; y < Y_Channel; y++) {
		...
		CTP_I2C_READ_1818C(ts_1818C->client, I2C_FW_Address, buf, X_Channel * 2 + 1);
	...
	CTP_I2C_WRITE_1818C(ts_1818C->client, I2C_FW_Address, buf, 3);
	buf[0] = (uint8_t)(raw_btn_pipe_addr & 0xFF);
	CTP_I2C_READ_1818C(ts_1818C->client, I2C_FW_Address, buf, Key_Channel * 2 + 1);
...
return:
	n.a.
 *******************************************************/
void print_selftest_result_1818C(struct seq_file *m, int32_t TestResult, uint8_t RecordResult[], int32_t rawdata[], uint8_t x_len, uint8_t y_len)
{
	int32_t i = 0;
	int32_t j = 0;
	int32_t iArrayIndex = 0;
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	if ((TestResult_Short == 0) || (TestResult_Short == 1)) {
		print_selftest_result_1818C(m, TestResult_Short, RecordResult_Short, RawData_Short, X_Channel, Y_Channel);
	} else { /* TestResult_Short is -1 */
		if (ts_1818C->carrier_system) {
			nvt_mp_seq_printf(m, " FAIL!\n");
			if (TestResult_Short_Diff == -1) {
				nvt_mp_seq_printf(m, "Short Diff");
				print_selftest_result_1818C(m, TestResult_Short_Diff, RecordResult_Short_Diff, RawData_Short_Diff, X_Channel, Y_Channel);
			}
			if (TestResult_Short_Base == -1) {
				nvt_mp_seq_printf(m, "Short Base");
				print_selftest_result_1818C(m, TestResult_Short_Base, RecordResult_Short_Base, RawData_Short_Base, X_Channel, Y_Channel);
			}
		} else {
			print_selftest_result_1818C(m, TestResult_Short, RecordResult_Short, RawData_Short, X_Channel, Y_Channel);
		}
	...
	print_selftest_result_1818C(m, TestResult_Open, RecordResult_Open, RawData_Open, X_Channel, Y_Channel);

	nvt_mp_seq_printf(m, "FW Rawdata Test");
	if ((TestResult_FW_Rawdata == 0) || (TestResult_FW_Rawdata == 1)) {
		 print_selftest_result_1818C(m, TestResult_FWMutual, RecordResult_FWMutual, RawData_FWMutual, X_Channel, Y_Channel);
		...
		if (TestResult_FW_CC == -1) {
			if (ts_1818C->carrier_system) {
				if (TestResult_FW_CC_I == -1) {
					...
					print_selftest_result_1818C(m, TestResult_FW_CC_I, RecordResult_FW_CC_I, RawData_FW_CC_I, X_Channel, Y_Channel);
				}
				if (TestResult_FW_CC_Q == -1) {
					nvt_mp_seq_printf(m, "FW CC_Q");
					print_selftest_result_1818C(m, TestResult_FW_CC_Q, RecordResult_FW_CC_Q, RawData_FW_CC_Q, X_Channel, Y_Channel);
				}
			} else {
				nvt_mp_seq_printf(m, "FW CC");
				print_selftest_result_1818C(m, TestResult_FW_CC, RecordResult_FW_CC, RawData_FW_CC, X_Channel, Y_Channel);
			}
		}
	}
	...
	if ((TestResult_Noise == 0) || (TestResult_Noise == 1)) {
		print_selftest_result_1818C(m, TestResult_FW_DiffMax, RecordResult_FW_DiffMax, RawData_Diff_Max, X_Channel, Y_Channel);
		...
		if (TestResult_FW_DiffMin == -1) {
			...
			print_selftest_result_1818C(m, TestResult_FW_DiffMin, RecordResult_FW_DiffMin, RawData_Diff_Min, X_Channel, Y_Channel);
		}
	}
...
const struct seq_operations nvt_selftest_seq_ops_1818C = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
...
{
	struct device_node *np = ts_1818C->client->dev.of_node;
...
/*		int  LCM_ID = 1;//2;	//VIVO should set LCM_ID in globle variable!
		...
		snprintf(mpcriteria, 32, "novatek-mp-criteria-5B09");
		nvt_mp_parse_dt_1818C(np, mpcriteria);
	} else {
	...
	if (nvt_check_fw_reset_state_1818C(RESET_STATE_NORMAL_RUN)) {
		mutex_unlock(&ts_1818C->lock);
	...
	nvt_change_mode_1818C(MP_MODE_CC);
	...
	return seq_open(file, &nvt_selftest_seq_ops_1818C);
}
#if (_CustomerFunction_)

int bbk_nt_sensor_test_1818C(char *buf, void *pdata, int tmp)
{
	int retval = 0;/*NO_ERR;*/
	struct device_node *np = ts_1818C->client->dev.of_node;
	unsigned char mpcriteria[32] = {0};	/*novatek-mp-criteria-default*/
...
return:
	n.a.
 *******************************************************/
void nvt_mp_parse_ain_1818C(struct device_node *np, const char *name, uint8_t *array, int32_t size)
{
...
return:
	n.a.
 *******************************************************/
void nvt_mp_parse_u32_1818C(struct device_node *np, const char *name, int32_t *para)
{
	int32_t ret;
...
return:
	n.a.
 *******************************************************/
void nvt_mp_parse_array_1818C(struct device_node *np, const char *name, int32_t *array,
		int32_t size)
{
	struct property *data;
	int32_t len, ret;
...
void nvt_mp_parse_dt_1818C(struct device_node *root, const char *node_compatible)
{
	...
	/* MP Config */
	nvt_mp_parse_u32_1818C(np, "IC_X_CFG_SIZE", &IC_X_CFG_SIZE);

	nvt_mp_parse_u32_1818C(np, "IC_Y_CFG_SIZE", &IC_Y_CFG_SIZE);

#if TOUCH_KEY_NUM > 0
	nvt_mp_parse_u32_1818C(np, "IC_KEY_CFG_SIZE", &IC_KEY_CFG_SIZE);
#endif

	nvt_mp_parse_u32_1818C(np, "X_Channel", &X_Channel);

	nvt_mp_parse_u32_1818C(np, "Y_Channel", &Y_Channel);

	nvt_mp_parse_ain_1818C(np, "AIN_X", AIN_X, IC_X_CFG_SIZE);

	nvt_mp_parse_ain_1818C(np, "AIN_Y", AIN_Y, IC_Y_CFG_SIZE);

#if TOUCH_KEY_NUM > 0
	nvt_mp_parse_ain_1818C(np, "AIN_KEY", AIN_KEY, IC_KEY_CFG_SIZE);
#endif

	/* MP Criteria */
	if (ts_1818C->carrier_system) {
		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_Short_Diff_P", PS_Config_Lmt_Short_Diff_P,
				X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_Short_Diff_N", PS_Config_Lmt_Short_Diff_N,
				X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_Short_Base_P", PS_Config_Lmt_Short_Base_P,
				X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_Short_Base_N", PS_Config_Lmt_Short_Base_N,
				X_Channel * Y_Channel + Key_Channel);
	} else {
		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_Short_Rawdata_P", PS_Config_Lmt_Short_Rawdata_P,
				X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_Short_Rawdata_N", PS_Config_Lmt_Short_Rawdata_N,
				X_Channel * Y_Channel + Key_Channel);
	}

	nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_Open_Rawdata_P", PS_Config_Lmt_Open_Rawdata_P,
			X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_Open_Rawdata_N", PS_Config_Lmt_Open_Rawdata_N,
			X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_Rawdata_P", PS_Config_Lmt_FW_Rawdata_P,
			X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_Rawdata_N", PS_Config_Lmt_FW_Rawdata_N,
			X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_CC_P", PS_Config_Lmt_FW_CC_P,
			X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_CC_N", PS_Config_Lmt_FW_CC_N,
			X_Channel * Y_Channel + Key_Channel);

	if (ts_1818C->carrier_system) {
		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_CC_I_P", PS_Config_Lmt_FW_CC_I_P,
				X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_CC_I_N", PS_Config_Lmt_FW_CC_I_N,
				X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_CC_Q_P", PS_Config_Lmt_FW_CC_Q_P,
				X_Channel * Y_Channel + Key_Channel);

		nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_CC_Q_N", PS_Config_Lmt_FW_CC_Q_N,
				X_Channel * Y_Channel + Key_Channel);
	}

	nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_Diff_P", PS_Config_Lmt_FW_Diff_P,
			X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_array_1818C(np, "PS_Config_Lmt_FW_Diff_N", PS_Config_Lmt_FW_Diff_N,
			X_Channel * Y_Channel + Key_Channel);

	nvt_mp_parse_u32_1818C(np, "PS_Config_Diff_Test_Frame", &PS_Config_Diff_Test_Frame);
 ...
 *******************************************************/
int32_t nvt_mp_proc_init_1818C(void)
{
	int ret = 0;
