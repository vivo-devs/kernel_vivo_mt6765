		...
		struct device_attribute *attr, char *buf)
{
	...
	int status = gpio_get_value(data->status_gpio);

	return scnprintf(buf, PAGE_SIZE, "%d\n", status);
}
static DEVICE_ATTR(simtray_status, 0444, simtray_status_show, NULL);
static int simtray_probe(struct platform_device *pdev)
{
	...
	ret = sysfs_create_file(&dev->kobj, &dev_attr_simtray_status.attr);
	if (ret < 0) {
...
{
	sysfs_remove_file(&pdev->dev.kobj, &dev_attr_simtray_status.attr);
	return 0;
}
...
static int __init simtray_status_driver_init(void)
{
	return platform_driver_register(&simtray_status_driver);
}

static void __exit simtray_status_driver_exit(void)
{
	platform_driver_unregister(&simtray_status_driver);
}

module_init(simtray_status_driver_init);
module_exit(simtray_status_driver_exit);

MODULE_DESCRIPTION("vivo SIM tray status");
MODULE_LICENSE("GPL");