...
extern unsigned int silentreboot;
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	if (level == old_brightness) {
		//printk("[ddp_dsi_ioctl]old_brightness is %d, level = %d\n", old_brightness, level);
		/*if (silentreboot == 1)*/
			return 1;
	} else {
		old_brightness = level;

		if ((0 == level) && (silentreboot == 1)) {
			silentreboot = 0;   /* when backligt set to 0, clear silentreboot */
			silent_count = 0;
			printk("[ddp_dsi_ioctl]silentreboot  is clear %d\n", silentreboot);
		}

		if (silentreboot == 1) {
			level = 0;
			silent_count++;
			printk ("[ddp_dsi_ioctl]silent_count is %d\n", silent_count);
		}
	}
	/* add for silent mode  end*/
