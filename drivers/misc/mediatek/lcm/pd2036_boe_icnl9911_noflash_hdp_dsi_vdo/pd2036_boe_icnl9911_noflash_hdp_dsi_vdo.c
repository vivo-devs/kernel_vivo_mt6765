...
/*#ifdef CONFIG_MTK_LCM_MIPI_CLK_CONTROL*/
	if (rf_hw_id == 1) {
		...
		printk("%s PD1987 rf_hw_id PLL_CLOCK %d\n", __func__, params->dsi.PLL_CLOCK);
	}
/*#endif*/
...
{
	LCM_INFO("%s icnl9911 -> enter power on begin, panel_reset_state=%d\n", __func__, panel_reset_state);
	...
	************************************/
		...
		LCM_INFO("%s icnl9911 -> enter power on end\n", __func__);
	}
}
...
static void lcm_suspend_power(void)
{
	...
	lcm_enp_setting(0);
	MDELAY(5);
	LCM_INFO("[LCM]lcm_suspend_power icnl9911- vddi keep high level\n");
	panel_reset_state = 0;// clear reset state
}

static void lcm_init(void)
{
	LCM_INFO("[LCM]icnl9911- init begin\n");
	lcm_reset_setting(1);
	MDELAY(20);
	lcm_reset_setting(0);
	MDELAY(12);
	...
	LCM_INFO("[LCM]icnl9911- init end\n");
}
...
static void lcm_suspend(void)
{
	push_table(lcm_suspend_setting,
		...
		1);
	panel_off_state = 1;
	LCM_INFO("[LCM]icnl9911- lcm_suspend display off\n");
}
...
struct LCM_DRIVER pd2036_boe_icnl9911_noflash_hdp_dsi_vdo_lcm_drv = {
	.name = "pd2036_boe_icnl9911_noflash_hdp_dsi_vdo",
