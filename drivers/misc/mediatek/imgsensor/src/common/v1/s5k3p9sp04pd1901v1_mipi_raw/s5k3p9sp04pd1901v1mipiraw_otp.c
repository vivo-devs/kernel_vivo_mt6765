...
#include "s5k3p9sp04pd1901v1mipiraw_Sensor.h"

#define PFX "S5K3P9SP04PD1901V1_EEPROM_OTP"
...
#define VIVO_OTP_DATA_SIZE 0x07DA /*sizeof(moduleinfo)+sizeof(awb)+sizeof(af)+sizeof(lsc)*/
...
static unsigned const int Lsc_addr = 0x008c;
static unsigned const int Lsc_checksum_addr = 0x07d9;
...
#endif
static int checksum;
otp_error_code_t S5K3P9SP04PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_sub_s5k3p9sp04pd1901v1[13];
...
int S5K3P9SP04PD1901V1_otp_read(void)
{
	int i = 0;
	...
	/*if (is_atboot == 1) {
		LOG_INF("AT mode skip S5K3P9SP04PD1901V1_otp_read\n");
		return 1;
	}
	*/
	/* guojunzheng add end */
	...
	if ((0x01 == vivo_otp_data[ModuleInfo_addr]) && (0x01 == vivo_otp_data[Sensor_fuse_id_addr]) && (0x01 == vivo_otp_data[SN_addr]) && (0x01 == vivo_otp_data[Awb_addr]) && (0x01 == vivo_otp_data[Awb_low_addr])&& (0x01 == vivo_otp_data[Lsc_addr])) {
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		if ((VIVO_VENDOR_TRULY != vivo_otp_data[0x01])&&(VIVO_VENDOR_OFILM != vivo_otp_data[0x01])){
			S5K3P9SP04PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("Module ID error!!!    otp_error_code:%d\n", S5K3P9SP04PD1901V1_OTP_ERROR_CODE);
			return 0;
		}
		...
		else if ((VIVO_VENDOR_LENS_ID != vivo_otp_data[0x07]) && (VIVO_VENDOR_VCM_ID != vivo_otp_data[0x08])&& (VIVO_VENDOR_DRIVERIC_ID != vivo_otp_data[0x09])) {
			S5K3P9SP04PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		...
		if (vivo_otp_data[ModuleInfo_checksum_addr] != checksum) {
			S5K3P9SP04PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_sub_s5k3p9sp04pd1901v1[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_sub_s5k3p9sp04pd1901v1[i +1] = (MUINT32)vivo_otp_data[i + SN_addr+1];
			/*LOG_INF("sn_inf_sub_s5k3p9sp04pd1901v1[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_sub_s5k3p9sp04pd1901v1[i +1],  i +SN_addr+1, vivo_otp_data[i + SN_addr+1]);*/
		}
		...
		if (t > 0) {
			S5K3P9SP04PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, S5K3P9SP04PD1901V1_OTP_ERROR_CODE);
			return 0;
		}
		/****check if awb out of range[low cct]****/
		R_unit_low = vivo_otp_data[Awb_low_addr+1];
		...
		R_golden_low = vivo_otp_data[Awb_low_addr+7];
		...
		if (diff_inf > 70 || diff_mac > 80 || diff_inf_macro > 450 || diff_inf_macro < 250) {  /*AF code out of range*/
			S5K3P9SP04PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
		...
		S5K3P9SP04PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
