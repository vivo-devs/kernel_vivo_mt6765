...
otp_error_code_t HI259_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int hi259_flag;
extern MUINT32  sn_inf_main_hi259[13];  /*0 flag   1-12 data*/
...
int hi259_vivo_otp_read(void)
{
	...
	HI259_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
		...
		{
			HI259_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("ModuleInfo_info error!!!    otp_error_code:%d\n",HI259_OTP_ERROR_CODE);
			return 0;
        ...
        {
			HI259_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_main_hi259[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_main_hi259[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_main_hi259[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_hi259[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//////AWB_checksum
		checksum = 0;
		...
		if(t > 0) 
		{
			HI259_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, HI259_OTP_ERROR_CODE);
			return 0;
		}
		return 1;
	} else {
		HI259_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
