...
#define PFX "OV13855PD1901_EEPROM_OTP"
...
otp_error_code_t OV13855PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int ov13855pd1901_flag;
...
int ov13855pd1901_vivo_otp_read(void)
{
	int i = 0;
	...
	OV13855PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (vivo_otp_data[0x07] != 0x01 ){
		OV13855PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		ov13855pd1901_flag = 0;
		LOG_INF("VCM ID Error!!!    ov13855pd1901_flag =%d\n", ov13855pd1901_flag);
	}else{
		ov13855pd1901_flag = 1;
		LOG_INF("ov13855pd1901_flag = %d\n", ov13855pd1901_flag);
	}
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
        ...
        {
			OV13855PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		if(t > 0) 
		{
			OV13855PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, OV13855PD1901_OTP_ERROR_CODE);
			return 0;
		}
        ...
        if(vivo_otp_data[0x07] == 0x02){    /* 0x02 Shicoh, 0x01 TDK */ 
			if (diff_inf > 100 || diff_mac > 120 || diff_inf_macro > 500 || diff_inf_macro < 260) {  /*AF code out of range*/
			    OV13855PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", OV13855PD1901_OTP_ERROR_CODE, vivo_otp_data[0x07], AF_inf, AF_mac);
			    return 0;
		    }
        }
		...
		{
			if(i == 0)
				LOG_INF("read_OV13855PD1901_PDAF_data");
			if(i < 496)
		...
		OV13855PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
