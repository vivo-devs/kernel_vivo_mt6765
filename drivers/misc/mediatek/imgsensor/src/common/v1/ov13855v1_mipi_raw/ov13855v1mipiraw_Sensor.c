 ...
 *============================================================================
 ****************************************************************************/
#define PFX "OV13855V1_camera_sensor"
...
extern int ov13855v1_vivo_otp_read(void);
static int vivo_otp_read_when_power_on = 0;
extern otp_error_code_t OV13855V1_OTP_ERROR_CODE;
extern unsigned int is_atboot;
extern unsigned  int ov13855v1_flag;
MUINT32 sn_inf_main_ov13855v1pd[13];  /*0 flag   1-12 data*/
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			//	pr_debug("add:start read eeprom  - when_power_on  = %d\n",vivo_otp_read_when_power_on);
				vivo_otp_read_when_power_on = ov13855v1_vivo_otp_read();
			//	pr_debug("add:end read eeprom - when_power_on = %d\n",vivo_otp_read_when_power_on);
			    /*vivo hope add end*/
				 //if(ov13855v1_flag == 1){
					pr_debug("i2c write id: 0x%x, sensor id: 0x%x, ov13855v1_flag = %d\n", imgsensor.i2c_write_id,*sensor_id, ov13855v1_flag);
					return ERROR_NONE;
				//}
			}
				pr_debug("Read sensor id fail, id: 0x%x or ov13855v1_flag = %d\n",
						imgsensor.i2c_write_id, ov13855v1_flag);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
	}
	if (*sensor_id != imgsensor_info.sensor_id || ov13855v1_flag != 1) {
        ...
        case SENSOR_FEATURE_GET_CUSTOM_INFO:
            switch (*feature_data) {
                case 0:    //info type: otp state
                    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = OV13855V1_OTP_ERROR_CODE;//otp_state
					memcpy(feature_data+2, sn_inf_main_ov13855v1pd, sizeof(MUINT32)*13);
					break;
            }
            break;
