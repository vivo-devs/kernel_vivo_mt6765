...
#define PFX "OV20880PD1901V3_EEPROM_OTP"
...
otp_error_code_t OV20880PD1901V3_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;

extern MUINT32 sn_inf_main_ov20880pd1901v3pd[13];  /*0 flag   1-12 data*/
extern MUINT32 crosstalk_datav3[607];  /*0 flag   1-606 data*/
extern MUINT32 dpc_datav3[837];  /*0 flag   1-836 data*/
...
int ov20880_read_sn_id_v3(void)
{
   int i = 0;
   int offset = 0x46;
...
int ov20880pd1901v3_vivo_otp_read(void)
{
	int i = 0;
	int offset = 0x0000;
	...
	OV20880PD1901V3_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
		...
		{
			OV20880PD1901V3_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("ModuleInfo_info error!!!    otp_error_code:%d\n",OV20880PD1901V3_OTP_ERROR_CODE);
			return 0;
		...
		else if((0xff != vivo_otp_data[0x100B]) || (0x00 != vivo_otp_data[0x100C]) || (0x0b != vivo_otp_data[0x100D]) || (0x01 != vivo_otp_data[0x100E]))
		{
		   ...
		   checksum = 0;
	        ...
	        {
				OV20880PD1901V3_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
	            LOG_INF("crosstalk_checksum error!!!   otp_error_code:%d\n",OV20880PD1901V3_OTP_ERROR_CODE);
				return 0;
	        }
			for (i = 0; i < 607; i++) {	
			crosstalk_datav3[i] = (MUINT32)vivo_otp_data[i + Crosstalk_addr];
			LOG_INF("crosstalk_datav3[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i, crosstalk_datav3[i],	i + Crosstalk_addr, vivo_otp_data[i + Crosstalk_addr]);
			}				
		   ...
		   checksum = 0;
			...
			for (i = 0; i < 837; i++) {	
			dpc_datav3[i] = (MUINT32)vivo_otp_data[i + Dpc_addr];
			LOG_INF("dpc_datav3[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i, dpc_datav3[i],	i + Dpc_addr, vivo_otp_data[i + Dpc_addr]);
			}					
		/*ModuleInfo_checksum*/
		checksum = 0;
		...
		sn_inf_main_ov20880pd1901v3pd[0] = 0x01;	
		#if 1	
		for (i = 0; i < 12; i++) {	
			sn_inf_main_ov20880pd1901v3pd[i + 1] = (MUINT32)vivo_otp_data[i + 1 + SN_addr];
			LOG_INF("sn_inf_main_ov20880pd1901v3pd[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i + 1, sn_inf_main_ov20880pd1901v3pd[i + 1],	i + SN_addr, vivo_otp_data[i + 1 + SN_addr]);	}	
	    #endif
		/*AWB_checksum*/
		checksum = 0;
		...
		if(t > 0) 
		{
			OV20880PD1901V3_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, OV20880PD1901V3_OTP_ERROR_CODE);
			return 0;
		}
		...
		if(vivo_otp_data[0x07] == 0x02){    /* 0x02 Shicoh, 0x01 TDK */ 
			if (diff_inf > 100 || diff_mac > 120 || diff_inf_macro > 500 || diff_inf_macro < 260) {  /*AF code out of range*/
			    OV20880PD1901V3_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", OV20880PD1901V3_OTP_ERROR_CODE, vivo_otp_data[0x07], AF_inf, AF_mac);
			    return 0;
		    }
		}
		...
		{
			if(i == 0)
				LOG_INF("read_OV20880PD1901V3_PDAF_data");
			if(i < 496)
		...
		OV20880PD1901V3_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
