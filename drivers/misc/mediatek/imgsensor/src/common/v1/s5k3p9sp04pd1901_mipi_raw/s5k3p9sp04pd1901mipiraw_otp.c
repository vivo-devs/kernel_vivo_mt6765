...
#include "s5k3p9sp04pd1901mipiraw_Sensor.h"

#define PFX "S5K3P9SP04PD1901_EEPROM_OTP"
...
#endif
static int checksum;
otp_error_code_t S5K3P9SP04PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_sub_s5k3p9sp04pd1901[13];
...
int S5K3P9SP04PD1901_otp_read(void)
{
	int i = 0;
	...
	/*if (is_atboot == 1) {
		LOG_INF("AT mode skip S5K3P9SP04PD1901_otp_read\n");
		return 1;
	}
	*/
	/* guojunzheng add end */
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		if ((VIVO_VENDOR_TRULY != vivo_otp_data[0x01])&&(VIVO_VENDOR_OFILM != vivo_otp_data[0x01])){
			S5K3P9SP04PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("Module ID error!!!    otp_error_code:%d\n", S5K3P9SP04PD1901_OTP_ERROR_CODE);
			return 0;
		}
		...
		if (vivo_otp_data[ModuleInfo_checksum_addr] != checksum) {
			S5K3P9SP04PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_sub_s5k3p9sp04pd1901[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_sub_s5k3p9sp04pd1901[i +1] = (MUINT32)vivo_otp_data[i + SN_addr+1];
			/*LOG_INF("sn_inf_sub_s5k3p9sp04pd1901[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_sub_s5k3p9sp04pd1901[i +1],  i +SN_addr+1, vivo_otp_data[i + SN_addr+1]);*/
		}
		...
		if (t > 0) {
			S5K3P9SP04PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, S5K3P9SP04PD1901_OTP_ERROR_CODE);
			return 0;
		}
		...
		if (diff_inf > 70 || diff_mac > 80 || diff_inf_macro > 450 || diff_inf_macro < 250) {  /*AF code out of range*/
			S5K3P9SP04PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
		...
		S5K3P9SP04PD1901_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
