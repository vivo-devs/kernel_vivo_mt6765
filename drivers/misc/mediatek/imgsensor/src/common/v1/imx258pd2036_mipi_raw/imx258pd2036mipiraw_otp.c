...
#define PFX "IMX258PD2036_EEPROM_OTP"
...
#define VIVO_OTP_DATA_SIZE 0x1B80 /*sizeof(moduleinfo)+sizeof(awb)+sizeof(af)+sizeof(lsc)*/
...
static unsigned const int PD_Proc2_checksum_addr = 0x19ef;
static unsigned const int Song_SPC_addr = 0x19F0;
static unsigned const int Song_SPC_checksum_addr = 0x1B7F;
#ifdef AF_RANGE_GOT
...
otp_error_code_t IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int imx258pd2036_flag;
extern MUINT32  sn_inf_main_imx258pd2036[13];  /*0 flag   1-12 data*/
...
int imx258pd2036_vivo_otp_read(void)
{
	int i = 0;
	...
	IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (vivo_otp_data[0x0A] != 0x05 ){
		IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		imx258pd2036_flag = 0;
		LOG_INF("VCM ID Error!!!    imx258pd2036_flag =%d\n", imx258pd2036_flag);
	}else{
		imx258pd2036_flag = 1;
		LOG_INF("imx258pd2036_flag = %d\n", imx258pd2036_flag);
	}
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
		...
		{
			IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("ModuleInfo_info error!!!    otp_error_code:%d\n",IMX258PD2036_OTP_ERROR_CODE);
			return 0;
		} else if (((vivo_otp_data[0x02] != 0x03) ||(vivo_otp_data[0x08] != 0x11) ||(vivo_otp_data[0x07] != 0x00) ||(vivo_otp_data[0x06] != 0x17)|| ((vivo_otp_data[0x09] != 0x09) && (vivo_otp_data[0x09] != 0x12)) || (vivo_otp_data[0x0A] != 0x05) || (vivo_otp_data[0x0B] != 0xFF)))
		{
			IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("Platform ID or Sensor or Lens or VCM ID or Driver_IC ID  Error!!!    otp_error_code:%d\n",IMX258PD2036_OTP_ERROR_CODE);
			return 0;
		} else if ((vivo_otp_data[0x0A] != 0x05)) {
			IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
        ...
        {
			IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_main_imx258pd2036[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_main_imx258pd2036[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_main_imx258pd2036[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_imx258pd2036[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//////AWB_checksum
		checksum = 0;
		...
		//////Sony_SPC_checksum
		checksum = 0;
        for(i = Song_SPC_addr+1; i < Song_SPC_checksum_addr; i ++)
        {              
			checksum += vivo_otp_data[i];						
        }  
			checksum = checksum % 0xff + 1;
		if( vivo_otp_data[Song_SPC_checksum_addr] != checksum )
        {
			IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
            LOG_INF("Sony_SPC_checksum error!!!   otp_error_code:%d\n",IMX258PD2036_OTP_ERROR_CODE);
			return 0;
        }
		...
		if(t > 0) 
		{
			IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, IMX258PD2036_OTP_ERROR_CODE);
			return 0;
		}
        ...
        if(vivo_otp_data[0x01] == 0x05){    
			...
			if (diff_inf_137 > 150 || diff_mac_137 > 150 || diff_inf_macro > 450 || diff_inf_macro < 150) {  /*AF code out of range*/
			    IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("QTech AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", IMX258PD2036_OTP_ERROR_CODE, vivo_otp_data[0x0A], AF_inf, AF_mac);
			    return 0;
		    }
        }
		    ...
		    if (diff_mac_158 > 110 || diff_mac_158 > 120 || diff_inf_macro > 470 || diff_inf_macro < 230) {  /*AF code out of range*/
			    IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("Truly AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", IMX258PD2036_OTP_ERROR_CODE, vivo_otp_data[0x0A], AF_inf, AF_mac);
			    return 0;
		    }
        }
		#endif
		...
		IMX258PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
