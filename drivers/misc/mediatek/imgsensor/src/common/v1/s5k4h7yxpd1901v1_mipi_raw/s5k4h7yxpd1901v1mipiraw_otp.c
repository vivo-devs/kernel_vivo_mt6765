...
#define PFX "S5K4H7YXPD1901V1_EEPROM_OTP"
...
unsigned char vivo_sub_otp_data_s5k4h7yxpd1901v1[1886];
...
static int checksum;
otp_error_code_t S5K4H7YXPD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_sub_s5k4h7yxpd1901v1[13];
...
static int s5k4h7yxpd1901v1_otp_read_setting_init_continuous_read(kal_uint16 addr)
{
	int i = 0, j = 0;
...
int S5K4H7YXPD1901V1_otp_read(void)
{
	int i = 0 ;
	//int j = 0;
	int offset = 0x0401;
	...
	S5K4H7YXPD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	s5k4h7yxpd1901v1_otp_read_setting_init_continuous_read(offset);
	LOG_INF("read_s5k4h7yxpd1901v1_data\n");

	#if 0
	for(j = 0; j < 62; j++){
		for (i = 0 ; i < 0x40; i++) {
			LOG_INF("read_s5k4h7yxpd1901v1_data[%d][0x%x][%d] = 0x%x\n", (j + 21), (i + 0x0A04), (i +0x40*j), vivo_sub_otp_data[i +0x40*j]);
		}
	}
	#endif
	...
	LOG_INF("read_s5k4h7yxpd1901v1_data,moduleinfo_addr=%d,awb_addr= %d, Lsc_sub_addr = %d, sn = %d, fuse id addr = %d\n", ModuleInfo_sub_addr, Awb_sub_addr, Lsc_sub_addr, Sn_sub_addr, Fuse_id_sub_addr);
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		if ((VIVO_SUB_VENDOR_TRULY != vivo_sub_otp_data[ModuleInfo_sub_addr + 0x00])&&(VIVO_SUB_VENDOR_SUNNY != vivo_sub_otp_data[ModuleInfo_sub_addr + 0x00])&&(VIVO_SUB_VENDOR_OFILM != vivo_sub_otp_data[ModuleInfo_sub_addr + 0x00])&&( VIVO_SUB_VENDOR_QTECH != vivo_sub_otp_data[ModuleInfo_sub_addr + 0x00]) ) {
			S5K4H7YXPD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("[cfx++]Module ID error!!!    otp_error_code:%d\n", S5K4H7YXPD1901V1_OTP_ERROR_CODE);
			return 0;
		...
		if (vivo_sub_otp_data[ModuleInfo_sub_checksum_addr] != checksum) {
			S5K4H7YXPD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_sub_s5k4h7yxpd1901v1[0] = 0x01;
		
		for (i = 0; i < 12; i++) {
			sn_inf_sub_s5k4h7yxpd1901v1[i +1] = (MUINT32)vivo_sub_otp_data[i + Sn_sub_addr];
			LOG_INF("sn_inf_sub_s5k4h7yxpd1901v1[%d] = 0x%x, vivo_sub_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_sub_s5k4h7yxpd1901v1[i+1],  i +Sn_sub_addr, vivo_sub_otp_data[i + Sn_sub_addr]);
		}
		...
		if (t > 0) {
			S5K4H7YXPD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("[cfx++]AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, S5K4H7YXPD1901V1_OTP_ERROR_CODE);
			return 0;
		}
		...
		#endif
		memcpy(vivo_sub_otp_data_s5k4h7yxpd1901v1, &vivo_sub_otp_data[ModuleInfo_sub_addr], 6);
		memcpy(&vivo_sub_otp_data_s5k4h7yxpd1901v1[6], &vivo_sub_otp_data[Awb_sub_addr], 12);
		memcpy(&vivo_sub_otp_data_s5k4h7yxpd1901v1[18], &vivo_sub_otp_data[Lsc_sub_addr], 1868);
		//add for printk S5K4H7YXPD1901V1
		#if 0
		for (i = 0; i < 1886; i++) {
			LOG_INF("read_otp_data_for_hal[%d] = 0x%x\n", i, vivo_sub_otp_data_s5k4h7yxpd1901v1[i]);
		}
		#endif
		return 1;
	} else {
		S5K4H7YXPD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
