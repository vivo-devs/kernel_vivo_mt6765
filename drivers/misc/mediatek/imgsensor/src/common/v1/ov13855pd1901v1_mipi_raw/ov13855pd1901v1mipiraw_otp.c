...
#define PFX "OV13855PD1901V1_EEPROM_OTP"
...
otp_error_code_t OV13855PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int ov13855pd1901v1_flag;
extern MUINT32 sn_inf_main_ov13855pd1901v1pd[13];  /*0 flag   1-12 data*/
...
int ov13855pd1901v1_vivo_otp_read(void)
{
	int i = 0;
	int offset = 0x0000;
	...
	OV13855PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if ((vivo_otp_data[0x1009] != 0x09)&&(vivo_otp_data[0x1009] != 0x07)){
		OV13855PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		ov13855pd1901v1_flag = 0;
		LOG_INF("VCM ID Error!!!    ov13855pd1901v1_flag =%d\n", ov13855pd1901v1_flag);
	}else{
		ov13855pd1901v1_flag = 1;
		LOG_INF("ov13855pd1901v1_flag = %d\n", ov13855pd1901v1_flag);
	}
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
		...
		checksum = 0;
	        ...
	        {
				OV13855PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_main_ov13855pd1901v1pd[0] = 0x01;	
		#if 1	
		for (i = 0; i < 12; i++) {	
			sn_inf_main_ov13855pd1901v1pd[i + 1] = (MUINT32)vivo_otp_data[i + 1 + SN_addr];
			LOG_INF("sn_inf_main_ov13855pd1901v1pd[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i + 1, sn_inf_main_ov13855pd1901v1pd[i + 1],	i + SN_addr, vivo_otp_data[i + 1 + SN_addr]);	}	
	    #endif
		/*AWB_checksum*/
		checksum = 0;
		...
		if(t > 0) 
		{
			OV13855PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, OV13855PD1901V1_OTP_ERROR_CODE);
			return 0;
		}
		...
		if(vivo_otp_data[0x07] == 0x02){    /* 0x02 Shicoh, 0x01 TDK */ 
			if (diff_inf > 100 || diff_mac > 120 || diff_inf_macro > 500 || diff_inf_macro < 260) {  /*AF code out of range*/
			    OV13855PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", OV13855PD1901V1_OTP_ERROR_CODE, vivo_otp_data[0x07], AF_inf, AF_mac);
			    return 0;
		    }
		}
		...
		{
			if(i == 0)
				LOG_INF("read_OV13855PD1901V1_PDAF_data");
			if(i < 496)
		...
		OV13855PD1901V1_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
