...
#define PFX "S5K3L6XXPD2074_EEPROM_OTP"
...
#define VIVO_VENDOR_LENS_ID 0x12
...
otp_error_code_t S5K3L6XX_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
extern MUINT32  sn_inf_main_s5k3l6xx[13];
unsigned  int s5k3l6xx_flag;
...
int s5k3l6xx_vivo_otp_read(void)
{
	int i = 0;
	...
	if ((vivo_otp_data[0x09] != VIVO_VENDOR_VCM_ID) ||(vivo_otp_data[0x0A] != VIVO_VENDOR_DRIVERIC_ID) ){
		S5K3L6XX_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		s5k3l6xx_flag = 0;
	...
	{
		/////ModuleInfo 
		if ((VIVO_VENDOR_QTECH != vivo_otp_data[0x01]) && (VIVO_VENDOR_TRULY != vivo_otp_data[0x01])) ///xuyongfu need check 2nd vendor
		{
			S5K3L6XX_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("ModuleInfo_info error!!!    otp_error_code:%d\n", S5K3L6XX_OTP_ERROR_CODE);
			return 0;
		} else if ((vivo_otp_data[0x02] != VIVO_VENDOR_PLATFORM_ID) || (vivo_otp_data[0x07] != 0xc6) || (vivo_otp_data[0x08] != VIVO_VENDOR_LENS_ID) || (vivo_otp_data[0x09] != VIVO_VENDOR_VCM_ID))
		{
			S5K3L6XX_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF(": Platform ID or Sensor or Lens or VCM ID or Driver_IC ID  Error!!!    otp_error_code:%d\n", S5K3L6XX_OTP_ERROR_CODE);
			return 0;
		} else if (vivo_otp_data[0x09] != VIVO_VENDOR_VCM_ID) {
			S5K3L6XX_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
        ...
        {
			S5K3L6XX_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_main_s5k3l6xx[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_main_s5k3l6xx[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			pr_info("sn_inf_main_s5k3l6xx[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_s5k3l6xx[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		...
		if (vivo_otp_data[0x08] == 0x07) {    /* 0x07 or 0x02 Shicoh, 0x01 TDK */ 
			if (diff_inf > 137 || diff_mac > 137 || diff_inf_macro > 511 || diff_inf_macro < 249) {  /*AF code out of range*/
			    S5K3L6XX_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", S5K3L6XX_OTP_ERROR_CODE, vivo_otp_data[0x07], AF_inf, AF_mac);
			    return 0;
		    }
        }
		...
		S5K3L6XX_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
