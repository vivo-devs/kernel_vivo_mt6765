...
#define VIVO_VENDOR_LENS_ID 0x00
...
otp_error_code_t HI259V1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int hi259v1_flag;
extern MUINT32  sn_inf_main_hi259v1[13];  /*0 flag   1-12 data*/
...
int hi259v1_vivo_otp_read(void)
{
	int i = 0;
	...
	HI259V1_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	//check_if_group_valid = vivo_otp_data[ModuleInfo_addr] | vivo_otp_data[Awb_addr] | vivo_otp_data[Af_addr] | vivo_otp_data[Lsc_addr] ;
	if((0x01 == vivo_otp_data[ModuleInfo_addr])  && (0x01 == vivo_otp_data[SNInfo_addr]) && (0x01 == vivo_otp_data[Awb_addr]) && (0x01 == vivo_otp_data[Lsc_addr]))
	{
		check_if_group_valid = 0x01;
		LOG_INF("0x01 is valid.check_if_group_valid:%d\n",check_if_group_valid);
	}
	
	if ((vivo_otp_data[0x06] == 0x02) && (vivo_otp_data[0x07] == 0x59) ){
	
		LOG_INF("Sensor ID H = 0x%x,Sensor ID M = 0x%x\n", vivo_otp_data[0x06],vivo_otp_data[0x07]);
	}
	if (check_if_group_valid == 0x01) //all the data is valid
	{
		/////ModuleInfo 
		if((VIVO_VENDOR_QTECH != vivo_otp_data[0x01]) && (VIVO_VENDOR_SHINE != vivo_otp_data[0x01]))
		{
			HI259V1_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("ModuleInfo_info error!!!    otp_error_code:%d\n",HI259V1_OTP_ERROR_CODE);
			return 0;
		} else if ((vivo_otp_data[0x02] != 0x03) ||(vivo_otp_data[0x08] != 0x07) ||(vivo_otp_data[0x07] != 0x59) ||(vivo_otp_data[0x06] != 0x02)|| (vivo_otp_data[0x09] != 0x00) || (vivo_otp_data[0x0A] != 0x00) || (vivo_otp_data[0x0B] != 0xFF))
		{
			HI259V1_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("Platform ID or Sensor or Lens or VCM ID or Driver_IC ID  Error!!!    otp_error_code:%d\n",HI259V1_OTP_ERROR_CODE);
			return 0;
		} else if ((vivo_otp_data[0x0A] != 0x00)) {
			HI259V1_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
        ...
        {
			HI259V1_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_main_hi259v1[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_main_hi259v1[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_main_hi259v1[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_hi259v1[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//////AWB_checksum
		checksum = 0;
		...
		if(t > 0) 
		{
			HI259V1_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, HI259V1_OTP_ERROR_CODE);
			return 0;
		}
		return 1;
	} else {
		HI259V1_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
