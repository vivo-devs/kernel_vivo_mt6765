...
//#include "hi1336sunny_mipi_raw_Sensor.h"
...
BYTE hi1336sunny_eeprom_data[DATA_SIZE] = {0};
...
static bool _read_eeprom(kal_uint16 addr, BYTE *data, kal_uint32 size)
{
	...
	for (i = 0; i < 496; i++) {
		...
		//LOG_INF("VCM1 read_eeprom_sunny 0x%0x %d\n",offset, data[i]);
		offset++;
		index++;
	}
	...
	for (i = 0; i < 908; i++) { //ok
		...
		//LOG_INF("VCM2 read_eeprom_sunny 0x%0x %d\n",offset, data[i]);
		offset++;
		index++;
	}
...
bool read_eeprom_sunny(kal_uint16 addr, BYTE *data, kal_uint32 size)
{
	BYTE af_pos[4];
	...
	LOG_INF("VCM read_eeprom_sunny Inf1 : 0x%0x%0x , Macro : 0x%0x%0x\n", af_pos[0], af_pos[1], af_pos[2], af_pos[3]);
