...
#define PFX "S5K4H7YX_EEPROM_OTP"
...
otp_error_code_t S5K4H7YX_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int S5K4H7YX_flag;
extern MUINT32  sn_inf_sub_s5k4h7yx[13];  /*0 flag   1-12 data*/
...
int S5K4H7YX_otp_read(void)
{
	...
	S5K4H7YX_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (vivo_otp_data[0x09] != 0x00 ){
		S5K4H7YX_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		S5K4H7YX_flag = 0;
		LOG_INF("VCM ID Error!!!    S5K4H7YX_flag =%d\n", S5K4H7YX_flag);
	}else{
		S5K4H7YX_flag = 1;
		LOG_INF("S5K4H7YX_flag = %d,VCM ID = 0x%x", S5K4H7YX_flag,vivo_otp_data[0x09]);
	}
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
        ...
        {
			S5K4H7YX_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_sub_s5k4h7yx[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_sub_s5k4h7yx[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_sub_s5k4h7yx[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_sub_s5k4h7yx[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//////AWB_checksum
		checksum = 0;
		...
		S5K4H7YX_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
