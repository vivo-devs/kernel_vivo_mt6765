...
#define PFX "OV13B10PD2036_EEPROM_OTP"
...
otp_error_code_t OV13B10PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
unsigned  int ov13b10pd2036_flag;
extern MUINT32  sn_inf_main_ov13b10pd2036[13];  /*0 flag   1-12 data*/
...
int ov13b10pd2036_vivo_otp_read(void)
{
	int i = 0;
	...
	OV13B10PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;
	...
	if (vivo_otp_data[0x0A] != 0x11 ){
		OV13B10PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
		ov13b10pd2036_flag = 0;
		LOG_INF("VCM ID Error!!!    ov13b10pd2036_flag =%d\n", ov13b10pd2036_flag);
	}else{
		ov13b10pd2036_flag = 1;
		LOG_INF("ov13b10pd2036_flag = %d\n", ov13b10pd2036_flag);
	}
	...
	if (check_if_group_valid == 0x01) //all the data is valid
	{
        ...
        {
			OV13B10PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
		...
		sn_inf_main_ov13b10pd2036[0] = 0x01;
		for(i = 0;i < 12;i++){
			sn_inf_main_ov13b10pd2036[i +1] = (MUINT32)vivo_otp_data[i + SNInfo_addr+1];
			LOG_INF("sn_inf_main_ov13b10pd2036[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_main_ov13b10pd2036[i +1],  i +SNInfo_addr+1, vivo_otp_data[i + SNInfo_addr+1]);
		}
		//////AWB_checksum
		checksum = 0;
		...
		if(t > 0) 
		{
			OV13B10PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[low cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, OV13B10PD2036_OTP_ERROR_CODE);
			return 0;
		}
        ...
        if(vivo_otp_data[0x01] == 0x05){    
			...
			if (diff_inf_137 > 150 || diff_mac_137 > 150 || diff_inf_macro > 450 || diff_inf_macro < 150) {  /*AF code out of range*/
			    OV13B10PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_AF_OUT_OF_RANGE;
			    LOG_INF("QTech 2311137 AF out of range error!!!   otp_error_code:%d   VCM ID = 0x%x AF_inf = %d AF_mac = %d\n", OV13B10PD2036_OTP_ERROR_CODE, vivo_otp_data[0x0A], AF_inf, AF_mac);
			    return 0;
		    }
        }
		...
		OV13B10PD2036_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
