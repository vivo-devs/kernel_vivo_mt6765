...
#ifdef CONFIG_TYPEC
	if (musb->is_active)
		queue_delayed_work(musb->st_wq, &musb->typec_work, 0);
#endif
#endif
}
...
#ifdef CONFIG_MTK_XHCI
#ifdef CONFIG_TYPEC
static void musb_typec_work(struct work_struct *work)
{
	struct musb *musb = container_of(work, struct musb, typec_work.work);

	if (!musb->typec_port) {
		pr_info("%s: %d: typec_port is null\n", __func__, __LINE__);
		return;
	}

	if (musb->is_active) {
		if (!musb->partner) {
			musb->partner = typec_register_partner(musb->typec_port,
							       &musb->partner_desc);
			pr_info("%s: %d: typec gadget connect\n", __func__, __LINE__);
		} else
			pr_info("%s: %d: typec gadget connect ignore\n", __func__, __LINE__);
	} else {
		if (musb->partner) {
			typec_unregister_partner(musb->partner);
			musb->partner = NULL;
			pr_info("%s: %d: typec gadget disconnect\n", __func__, __LINE__);
		} else
			pr_info("%s: %d: typec gadget disconnect ignore\n", __func__, __LINE__);
	}
}
#endif
#endif
...
#ifdef CONFIG_MTK_XHCI
#ifdef CONFIG_TYPEC
	cancel_delayed_work_sync(&musb->typec_work);
	if (musb->typec_port && musb->partner) {
		typec_unregister_partner(musb->partner);
		musb->partner = NULL;
	}
	if (musb->typec_port)
		typec_unregister_port(musb->typec_port);
#endif
#endif
...
#ifdef CONFIG_MTK_XHCI
#ifdef CONFIG_TYPEC
	musb->typec_caps.type = TYPEC_PORT_SNK;
	musb->typec_caps.data = TYPEC_PORT_UFP;
	musb->typec_caps.revision = 0x0120;
	musb->partner_desc.usb_pd = false;
	musb->partner_desc.accessory = TYPEC_ACCESSORY_NONE;
	musb->typec_port = typec_register_port(musb->controller, &musb->typec_caps);
	if (IS_ERR(musb->typec_port)) {
		pr_err("%s: %d: failed to register typec_port: %d\n",
		       __func__, __LINE__, PTR_ERR(musb->typec_port));
		musb->typec_port = NULL;
	}
	INIT_DELAYED_WORK(&musb->typec_work, musb_typec_work);
#endif
#endif
