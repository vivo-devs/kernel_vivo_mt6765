...
int write_dts_val_to_reg(char *reg_str, u8 offset, u8 shift)
{
	const char *con_val;
	char *val;
	int ret = 1;
	struct device_node *node = NULL;
	printk("%s: usb eye reg:%s, offset: %d, shit:%d\n", __func__, reg_str, offset, shift);
	...
	ret = of_property_read_string(node, reg_str, &con_val);
	if (ret)
		return 0;
	printk("%s: usb eye reg:%s, con_val= %s\n", __func__, reg_str, con_val);

	val = kzalloc(TEMP_BUF_SIZE, GFP_KERNEL);
	if (!val)
		return 0;
	strcpy(val, con_val);
	/* USB_DRIVING_CAPABILITY */
	usb20_phy_debugfs_write_width3(offset, shift, val);
	printk("%s: write val:%s to reg, done\n", __func__, val);
	kfree(val);
...
void mtk_usb_phy_tuning(void)
{
	...
	if (mtk_musb->is_host_mode) {
		/* USB_DRIVING_CAPABILITY */
		ret = write_dts_val_to_reg("host_term_vref_sel", OFFSET_RG_USB20_TERM_VREF_SEL, SHFT_RG_USB20_TERM_VREF_SEL);
		if (!ret)
			usb20_phy_debugfs_write_width3(OFFSET_RG_USB20_TERM_VREF_SEL, SHFT_RG_USB20_TERM_VREF_SEL, "100");

		ret = write_dts_val_to_reg("host_vrt_vref_sel", OFFSET_RG_USB20_VRT_VREF_SEL, SHFT_RG_USB20_VRT_VREF_SEL);
		if (!ret)
		...
		/* USB_DRIVING_CAPABILITY */
		ret = write_dts_val_to_reg("device_term_vref_sel", OFFSET_RG_USB20_TERM_VREF_SEL, SHFT_RG_USB20_TERM_VREF_SEL);
		if (!ret)
			usb20_phy_debugfs_write_width3(OFFSET_RG_USB20_TERM_VREF_SEL, SHFT_RG_USB20_TERM_VREF_SEL, "100");

		ret = write_dts_val_to_reg("device_vrt_vref_sel", OFFSET_RG_USB20_VRT_VREF_SEL, SHFT_RG_USB20_VRT_VREF_SEL);
		if (!ret)
