...
/* added for motion recognition */
static int accelhub_ReadSensorData_vivo(char *buf, int bufsize)
{
	struct accelhub_ipi_data *obj = obj_ipi_data;
	uint64_t time_stamp = 0;
	struct data_unit_t data;
...
static ssize_t data_show(struct device_driver *ddri, char *buf)
{
	...
	accelhub_ReadSensorData_vivo(strbuf, ACCELHUB_BUFSIZE);
	return snprintf(buf, PAGE_SIZE, "%s\n", strbuf);
}
...
static void scp_init_work_done(struct work_struct *work)
{
	...
	int32_t cmd_args[VIVO_SENSOR_COMMAND_ARGS_SIZE] = {0};
#endif
...
#ifdef CONFIG_VIVO_SENSOR_SCP_RESET_ENABLE
	pr_info("acc_cali: %d %d %d\n", acc_cali[0], acc_cali[1], acc_cali[2]);
	if (acc_cali[0] != 0 || acc_cali[1] != 0 || acc_cali[2] != 0) {
		cmd_args[0] = SENSOR_COMMAND_ACCEL_SET_ENG_CALI_DATA;
		cmd_args[1] = acc_cali[0];
		cmd_args[2] = acc_cali[1];
		cmd_args[3] = acc_cali[2];
		err = sensor_set_vivo_cmd_to_hub(ID_ACCELEROMETER, cmd_args, ARRAY_SIZE(cmd_args));
		if (err < 0)
...
static int gsensorhub_factory_do_vivo_command(uint8_t sensorType, int32_t *args, int args_len)
{
	int err = 0;
...
static struct accel_factory_fops gsensor_factory_fops = {
	...
	/* add by vsen team : acc_vivo_command begin */
	.do_vivo_commands = gsensorhub_factory_do_vivo_command,
	/* add by vsen team : acc_vivo_command end */
};
