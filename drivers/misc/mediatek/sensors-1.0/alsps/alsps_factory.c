					 ...
					 unsigned long arg)
{
	...
	case ALSPS_IOCTL_SET_ENG_MODE:
		...
		case ALSPS_IOCTL_NOTIFY_PS_BROKEN:
			...
			return 0;
		case ALSPS_IOCTL_DISABLE_ALS_PS:
			if (copy_from_user(&cmd_args, ptr, sizeof(cmd_args)))
				return -EFAULT;
			ALSPS_PR_ERR("ALSPS_IOCTL_DISABLE_ALS_PS %d %d\n", cmd_args[0], cmd_args[1]);
			if (alsps_factory.fops != NULL && alsps_factory.fops->do_vivo_commands != NULL) {
				cmd_args[0] = SENSOR_COMMAND_DISABLE_ENABLE_PROX;
				err = alsps_factory.fops->do_vivo_commands(ID_PROXIMITY, cmd_args, ARRAY_SIZE(cmd_args));
				if (err < 0) {
					ALSPS_PR_ERR("ALSPS_IOCTL_DISABLE_ALS_PS fail!\n");
					return -EINVAL;
				}
				copy_to_user(ptr, &cmd_args, sizeof(cmd_args));
				ALSPS_PR_ERR("ALSPS_IOCTL_DISABLE_ALS_PS args:(%d ,%d, %d)\n", cmd_args[0], cmd_args[1], cmd_args[2]);
			} else {
				ALSPS_PR_ERR("ALSPS_IOCTL_DISABLE_ALS_PS NULL\n");
				return -EINVAL;
			}
			return 0;
