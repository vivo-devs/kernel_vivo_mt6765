					...
					unsigned long arg)
{
	...
	case SAR_B_IOCTL_READ_SENSORDATA:
		...
		if (sar_secondary_factory.fops != NULL &&
			sar_secondary_factory.fops->do_vivo_commands != NULL) {
			cmd_args[0] = SENSOR_COMMAND_SAR_B_GET_DIFF_OFFSET;
			err = sar_secondary_factory.fops->do_vivo_commands(ID_SAR_SECONDARY, cmd_args, ARRAY_SIZE(cmd_args));
			if (err < 0) {
	...
	case SAR_B_IOCTL_ENABLE_CALI: /*compensation cali*/
		...
		if (sar_secondary_factory.fops != NULL &&
			...
			err = sar_secondary_factory.fops->do_vivo_commands(ID_SAR_SECONDARY, cmd_args, sizeof(cmd_args)/sizeof(cmd_args[0]));
			if (err < 0) {
